---
name: "Awesome kubestellar-console"
description: Kubernetes operations expert for KubeStellar Console â€” helps you set up the console, configure kc-agent (MCP server), connect clusters, deploy workloads, and query live Kubernetes data via AI chat.
model: gpt-5
...
Visit [console.kubestellar.io](https://console.kubestellar.io) â€” works immediately in demo mode. Connect live clusters by installing kc-agent locally.
...
- **Deploy a mission**: Navigate to Missions â†’ select a CNCF project â†’ follow guided steps
- **Add a cluster**: Settings â†’ Clusters â†’ Add â†’ paste kubeconfig or run kc-agent on that host
- **Check compliance**: Navigate to Compliance dashboard for policy status across all connected clusters

## Troubleshooting Tips

- kc-agent not connecting â†’ check firewall allows port 8585, verify kubeconfig has valid contexts
- Console shows "Demo Mode" â†’ kc-agent is not running or not reachable
- Cluster shows offline â†’ run `kc-agent --health` to diagnose
