---
name: "Awesome ai-readiness-reporter"
description: 'Runs the AgentRC readiness assessment on the current repository and produces a self-contained, static HTML dashboard at reports/index.html. Explains every readiness pillar, the maturity level, and an actionable remediation plan, framed by AgentRC measure â†’ generate â†’ maintain loop. Use when asked to assess, audit, score, report on, or visualise the AI readiness of a repo.'
...
> **Measure â†’ Generate â†’ Maintain.** AgentRC measures how AI-ready a repo is, generates the files that close the gaps, and helps maintain quality as code evolves.
...
3. **Read repo context** â€” load `.github/copilot-instructions.md`, `AGENTS.md`, `CLAUDE.md`, `agentrc.config.json`, and any policy JSON referenced. This lets you describe the *current state* per pillar precisely (e.g. "AGENTS.md present, 412 lines, last modified 3 weeks ago").
...
- **High** â€” directly steers what an AI agent generates or how it self-checks.
- **Medium** â€” influences agent output quality but indirectly.
- **Low** â€” general engineering hygiene with weaker AI leverage.
...
| **Code Quality** | Medium | Formatter config (Prettier/Biome) | A formatter config means the agent's output lands pre-formatted â€” no diff noise, no review comments about whitespace. Without it, AI-generated PRs trigger style discussions that drown out real feedback. |
...
|---|---|---|---|
| **AI Tooling** | High | Custom instructions (`.github/copilot-instructions.md`, `AGENTS.md`, `CLAUDE.md`), MCP servers, agent configs, AI skills | The direct interface between repo and AI agents â€” the highest-leverage pillar in the entire model. A good `AGENTS.md` is worth more than every other pillar combined: it tells the agent your stack, conventions, build commands, test commands, and review expectations in one place. MCP servers and custom skills extend the agent's reach into your tools. |

At Level 2+, AgentRC also checks **instruction consistency** â€” flag any divergence between multiple instruction files and recommend consolidation (preferring `AGENTS.md`).
...
Show extras in their own section. Mark each as âœ… present or â—» missing â€” never as a "failure".
...
3. **Honour overrides** â€” use the override `impact` and `level` rather than the defaults when bucketing findings.
4. **Surface thresholds** â€” if `thresholds.passRate` is set, compare the actual pass rate to it and show pass/fail prominently.
...
|---|---|
| ðŸ”´ **Fix First** | impact âˆˆ {critical, high} **and** the fix is small (single file or config) |
| ðŸŸ¡ **Fix Next** | impact = medium **and** the fix is small |
| ðŸ”µ **Plan** | impact = medium **and** larger refactor required |
| âšª **Backlog** | impact âˆˆ {low, info} |

When in doubt, prefer the higher bucket if the pillar is `Docs`, `Testing`, `Build`, or `AI Tooling` â€” these are the highest-leverage for AI agents.
...
`Score = 1 - (total deductions / max possible weight)`. Grades: A â‰¥ 0.9, B â‰¥ 0.8, C â‰¥ 0.7, D â‰¥ 0.6, F < 0.6.

---

## HTML Template â€” DO NOT IMPROVISE
...
- Preserve the embedded `<script type="application/json" id="raw-data">â€¦</script>` block so the report is self-describing.
- **Escape every substituted value** before inserting it into the template:
  ...
  - For `{{rawJsonCompact}}` (which lives inside the `<script type="application/json">` block), replace any `</script` substring with `<\/script` to prevent the script tag from being closed early. Do NOT HTML-escape inside this block â€” the JSON must remain valid.
  - Never substitute raw user-controlled strings (filenames, commit messages, recommendations) without escaping. A repo with `<img onerror=â€¦>` in a filename must NOT produce executable HTML in the report.
...
| `{{passRate}}` / `{{threshold}}` | pass rate vs policy threshold, fully-formatted (e.g. `85%` or `â€”` if N/A). The literal `%` is part of the substituted value, not the template. |
...
| Placeholder | Source |
|---|---|
| `{{pillarName}}` | "Style", "Build", "Testing", â€¦ |
| `{{pillarScore}}` | integer percent for this pillar |
| `{{pillarStatus}}` | `good` / `warn` / `bad` (drives the bar + dot colour) |
| `{{pillarRelevance}}` | `high` / `medium` / `low` â€” AI relevance from the table above |
...
1. **Always run `agentrc readiness --json`** â€” never fabricate data.
2. **Always render via the bundled `report-template.html`** (in the `acreadiness-assess` skill folder) â€” load the template, substitute placeholders, write to `reports/index.html`. Don't author HTML from scratch.
3. **Explain every pillar** â€” use the full per-pillar paragraph from the table above, plus *current state* and *specific recommendation*. No one-liners.
4. **Tag each pillar with its AI relevance** (`high` / `medium` / `low`) so the badge matches the table above.
5. **Connect every Repo Health finding to AI impact** â€” repo health is not generic devops here; frame it through how it helps Copilot and other agents.
6. **Honour policies** â€” if a policy is in scope, reflect its disable/override/threshold rules in the rendered report.
7. **Show extras separately** â€” they never affect the score; never list them as gaps.
8. **Frame next steps via AgentRC's loop** â€” Measure (this report) â†’ Generate (`agentrc instructions`) â†’ Maintain (CI `--fail-level`).
9. **Only write `reports/index.html`** â€” do not modify any other files. Create the `reports/` directory if missing.
10. **No fluff** â€” every paragraph in the report must add concrete information.
