  ...
  const cards: StatCard[] = [
    {
      ...
      accent: readinessScore >= 80 ? 'text-emerald-600 dark:text-emerald-400' : readinessScore >= 60 ? 'text-amber-600 dark:text-amber-400' : 'text-rose-600 dark:text-rose-400',
    },
    {
      label: 'Predicted OET',
      value: predictedScore > 0 ? String(predictedScore) : '—',
      sub: predictedScore >= 350 ? 'Pass band' : predictedScore > 0 ? 'Below pass' : 'No data yet',
      accent: predictedScore >= 350 ? 'text-emerald-600 dark:text-emerald-400' : 'text-muted',
    },
    {
      label: 'Days to Exam',
      ...
      accent: daysToExam != null && daysToExam <= 14 ? 'text-rose-600 dark:text-rose-400' : 'text-muted',
    },
    {
      label: 'Streak',
