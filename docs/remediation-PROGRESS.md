...
| 6 | Fix encoding artifacts across affected files | Done | Fixed `&quot;` rendering issue in `app/(auth)/terms/page.tsx` lead strings. Reverted JSX text `&quot;` to proper entities (React decodes these correctly in JSX text). No unicode corruption (`â€™` etc.) found. |
