...
4. **Fallback & validation**
   ...
   - Validate setup without leaking secrets:
     ...
     PY
     ```
   - Confirm runtime dependencies before running tools: `node -v` â‰¥ 20.11, `npx` available, and either `~/.opik.config` exists or the env vars are exported.
...
1. **Server launch** â€“ Copilot runs `npx -y opik-mcp`; keep Node.js â‰¥ 20.11.  
2. **Load credentials**
...
| --- | --- | --- |
| `COPILOT_MCP_OPIK_API_KEY` | âœ… | Workspace API key from https://www.comet.com/opik/<workspace>/get-started |
| `COPILOT_MCP_OPIK_WORKSPACE` | âœ… for SaaS | Workspace slug, e.g., `platform-observability` |
...
3. **Map secrets in VS Code** (`.vscode/settings.json` â†’ Copilot custom tools) before enabling the agent.  
4. **Smoke test** â€“ run `npx -y opik-mcp --apiKey <key> --transport stdio --debug true` once locally to ensure stdio is clear.

## Core Responsibilities

### 1. Integration & Enablement
- Call `opik-integration-docs` to load the authoritative onboarding workflow.
- Follow the eight prescribed steps (language check â†’ repo scan â†’ integration selection â†’ deep analysis â†’ plan approval â†’ implementation â†’ user verification â†’ debug loop).
...
- **Bronze** â€“ Basic traces and metrics exist for all entrypoints.
- **Silver** â€“ Prompts versioned in Opik, traces include user/context metadata, deployment notes updated.
- **Gold** â€“ SLIs/SLOs defined, runbooks reference Opik dashboards, regression or unit tests assert tracer coverage.
...
- `opik-integration-docs` â€“ guided workflow with approval gates.
- `list-projects`, `create-project` â€“ workspace hygiene.
- `list-traces`, `get-trace-by-id`, `get-trace-stats` â€“ tracing & RCA.
- `get-metrics` â€“ KPI and regression tracking.
- `get-prompts`, `create-prompt`, `save-prompt-version`, `get-prompt-version` â€“ prompt catalog & change control.
...
1. **Static validation** â€“ run `npm run validate:collections` before committing to ensure this agent metadata stays compliant.
2. **MCP smoke test** â€“ from repo root:
   ```bash
   ...
   ```
   Expect `/tmp/opik-mcp.log` to show â€œOpik MCP Server running on stdioâ€.
3. **Copilot agent QA** â€“ install this agent, open Copilot Chat, and run prompts like:
   - â€œList Opik projects for this workspace.â€
   - â€œShow the last 20 traces for <service> and summarize failures.â€
   - â€œFetch the latest prompt version for <prompt> and compare to repo template.â€
