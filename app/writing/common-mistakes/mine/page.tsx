...
export default function WritingMyMistakesPage() {
  ...
  return (
    <LearnerDashboardShell pageTitle="Your Common Mistakes">
      <div className="space-y-6">
        <LearnerPageHero
          eyebrow="Personalised"
          icon={TrendingUp}
          accent="amber"
          title="The mistakes showing up most in your letters"
          description="Tracks across the last 20 graded letters. Tap a card to read the lesson and the canon rule."
          highlights={[
            { icon: AlertCircle, label: 'Tracked', value: `${rows.length}` },
            { icon: TrendingUp, label: 'Total occurrences', value: `${totalCount}` },
          ]}
        />
        ...
        {top5.length > 0 ? (
          <section aria-labelledby="top5-heading" className="rounded-2xl border border-amber-200/60 bg-amber-50/40 p-5 shadow-sm">
            <h2 id="top5-heading" className="text-base font-bold text-amber-900">Top 5 to fix this week</h2>
            <p className="mt-1 text-xs text-amber-800">Knock these down and your next letter's C4 and C6 will jump.</p>
              ...
              {top5.map((row, idx) => (
                <li key={row.id} className="flex items-center justify-between gap-3 rounded-lg border border-amber-200/60 bg-white p-3">
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      <p className="text-sm font-bold text-navy">{row.summary}</p>
                      <p className="text-xs text-muted">{row.stat.occurrenceCount}× recently</p>
                    </div>
                  </div>
                  {row.canonRuleId ? (
                    <Button asChild size="sm" variant="outline">
                      <Link href={`/writing/canon/${encodeURIComponent(row.canonRuleId)}`}>Read rule</Link>
                    </Button>
                  ) : null}
                </li>
              ))}
          ...
          <Card padding="lg">
            <CardContent>
              <p className="text-sm text-muted">
                You don't have any tracked mistakes yet. Submit a letter and the system will start surfacing the patterns it catches.
              </p>
              <div className="mt-3">
                <Button asChild><Link href="/writing/practice/library">Browse the practice library</Link></Button>
              </div>
            </CardContent>
          </Card>
        )}
        ...
        {rows.length > 0 ? (
          <section aria-labelledby="all-mine-heading">
            <h2 id="all-mine-heading" className="mb-3 text-base font-bold text-navy">Every mistake we've tracked</h2>
