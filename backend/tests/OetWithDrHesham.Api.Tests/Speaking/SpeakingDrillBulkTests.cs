...
public sealed class SpeakingDrillBulkTests : IAsyncLifetime
{
    ...
    public Task InitializeAsync()
    {
        ...
        var stripe = new OetWithDrHesham.Api.Services.StripeGateway(
            new HttpClient(),
            billingOptions,
            TestRuntimeSettingsProvider.FromBillingOptions(billingOptions.Value),
            Microsoft.Extensions.Logging.Abstractions.NullLogger<OetWithDrHesham.Api.Services.StripeGateway>.Instance);
        var paypal = new OetWithDrHesham.Api.Services.PayPalGateway(new HttpClient(), billingOptions);
        var paymentGateways = new OetWithDrHesham.Api.Services.PaymentGatewayService(
            stripe,
            paypal,
