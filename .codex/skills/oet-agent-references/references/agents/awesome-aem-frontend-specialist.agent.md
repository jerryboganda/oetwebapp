...
name: "Awesome aem-frontend-specialist"
model: 'GPT-4.1'
...
- Document token mappings for team consistency: Figma 65px Cal Sans â†’ `text-h2-mobile md:text-h2 font-display`
...
<article class="cmp-card bg-white rounded-lg p-6 hover:shadow-lg transition-shadow duration-300"
  ...
  <div class="cmp-card__actions" data-sly-test="${model.ctaUrl}">
    <a href="${model.ctaUrl}"
      ...
      <span class="cmp-button__icon" aria-hidden="true">â†’</span>
    </a>
  </div>
</article>
...
# Figma Token: "Desktop/Title/H1" â†’ 75px, Cal Sans font
# Design System: text-h1-mobile md:text-h1 font-display
# Validation: 75px âœ“, Cal Sans âœ“

# Figma Token: "Desktop/Paragraph/P Body Big" â†’ 22px, Helvetica
...
Design System Match:
  ...
  Validation: âœ… Pixel value matches + Font family matches

# Wrong Approach:
Figma "H2" â†’ CSS "text-h2" (blindly matching names without validation)

# Correct Approach:
Figma 65px Cal Sans â†’ Find CSS classes that produce 65px Cal Sans â†’ text-h2-mobile md:text-h2 font-display
```
...
- Maintain a mapping table: Figma Token â†’ Pixel Value â†’ CSS Class
