...
| `ios/App/Podfile.lock` is not committed | Never generated in the repo. | The transitive pod graph (`KeychainSwift`, `IONFilesystemLib`, `GCDWebServer`) re-resolves on every build. CI now records the resolved lock and warns. |
