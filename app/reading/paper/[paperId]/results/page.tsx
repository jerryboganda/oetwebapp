...
import {
  getReadingAttemptAiExplanation,
...
function GroundedReadingExplanation({
  attemptId,
  item,
  unanswered,
}: {
  ...
  const [result, setResult] = useState<ReadingGroundedAiExplanationDto | null>(null);
  ...
  const loadExplanation = async () => {
    ...
    setError(null);
    try {
      setResult(await getReadingAttemptAiExplanation(attemptId, item.questionId));
    } catch (err) {
  ...
  return (
    <div className="mt-4 rounded-xl border border-indigo-200 bg-indigo-50/70 p-4 dark:border-indigo-400/30 dark:bg-indigo-950/20" data-testid="reading-grounded-ai">
