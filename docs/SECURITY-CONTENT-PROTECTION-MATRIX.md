...
|---|---|---|
| Screenshot protection | **Implemented** (Windows/Android) / **CI-proven, physical-Mac proof pending** (macOS) / **Compensated limitation** (iOS/web) | Windows/Android block capture at OS level. **macOS**: `NSWindow.sharingType=.none` hid the window from `screencapture -x` and ScreenCaptureKit stills on two GitHub-hosted macOS runners (macOS 15.7.9 and 26.6.2, run [35136291136](https://github.com/jerryboganda/oetwebapp/actions/runs/35136291136), 16 Sep 2026) — real Apple hardware, but not a phone-filmed physical Mac in normal use, which is the owner's stated acceptance bar (17 Sep 2026). iOS detects a still capture, immediately pauses/mutes playback, logs it, and revokes the playback session. Web has no capture API, so it uses the moving identifying watermark, short token, session enforcement, and audit controls. |
| Screen-recording protection | **Implemented** (Windows/Android/iOS) / **NOT SHIPPED — release held** (macOS) | Windows/Android/iOS blank the app during active recording/mirroring. **macOS does not meet this requirement as currently built.** The same CI run above captured the protected window in a `screencapture -V` recording on macOS 15.7.9 (visible) while hiding it on macOS 26.6.2 (hidden) — the same mechanism, OS-version-dependent, on Apple's own hardware. Apple's current documentation for `NSWindowSharingNone` states: *"A legacy constant that macOS no longer uses… Don't use this value to hide or omit content from being captured. Instead, use FairPlay Streaming (FPS)."* The owner (17 Sep 2026) has held release of any macOS desktop build until screen-recording protection is proven Pass, on a phone-filmed physical Mac, on every officially supported macOS version. See §3a and the FairPlay/DRM pre-implementation plan (project memory `oet-macos-video-079-0710-2026-09-16`). |
...
| DRM and encrypted streaming | **Required for macOS (owner gate, 17 Sep 2026); not yet enabled anywhere** | Clients receive only a 5-minute token-authenticated Bunny embed; raw HLS/MP4 URLs are no longer exposed. That player is the required MediaCage surface. FairPlay via Bunny MediaCage **Enterprise** (not Basic — Basic has no screen-grab protection) is now the recommended route to meet the macOS screen-recording requirement; pending owner approval of cost/plan and Apple FPS credential approval before implementation. See §4. |
...
| **macOS (Tauri)** | Blocked (CI-proven, macOS 15.7.9 + 26.6.2; physical-Mac proof pending) | **NOT reliably blocked** — VISIBLE on macOS 15.7.9, blocked on macOS 26.6.2 (same mechanism, real CI hardware, run [35136291136](https://github.com/jerryboganda/oetwebapp/actions/runs/35136291136)) | `NSWindow.sharingType = .none` (`src-tauri/src/commands.rs`). Apple's own docs call this "a legacy constant that macOS no longer uses" for this purpose and direct developers to FairPlay Streaming instead. | **NOT SHIPPED — release held by owner (17 Sep 2026) pending FairPlay DRM.** Do not mark this row Shipped again until screen recording passes on a phone-filmed physical Mac on every officially supported macOS version. |
...
**OBS / third-party capture tools, remote desktop, HDMI capture, camera-at-screen:** OBS and generic screen-recording tools ride the same OS capture APIs as the built-in recorder on Windows/Android, so they are blocked identically there. **On macOS this is now unproven, not assumed** — see §3a; ScreenCaptureKit-based tools (OBS, Zoom, Google Meet in Chrome, Microsoft Teams) are documented by Apple staff to behave inconsistently with `NSWindowSharingNone`, and Zoom/Meet have not been tested at all. Remote-desktop protocols that use a different scan-out path than the standard capture API (some RDP shadowing modes) are a known theoretical gap — not separately tested. A physical camera pointed at a screen, or an HDMI capture card between the GPU and a monitor, defeats **any** software control on **any** platform; the moving, identifying watermark is the sole compensating control for that class of attack, on every platform, by design.

## 3a. macOS screen-recording gap (owner release hold, 17 Sep 2026)

`NSWindow.sharingType = .none` was the only macOS capture-protection mechanism shipped through desktop 0.7.10. It is **not sufficient** to meet the security requirement:

- **Apple's own documentation says not to use it for this purpose.** The current `NSWindow.SharingType.none` reference page: *"A legacy constant that macOS no longer uses… Don't use this value to hide or omit content from being captured. Instead, use FairPlay Streaming (FPS)."*
- **Apple engineering support confirms it in writing**, on two separate forum threads: *"Virtually all screen recording/remote control app operate below the window level, which means kCGWindowSharingState has no effect. The behavior here is basically deliberate,"* and, asked specifically about ScreenCaptureKit on current macOS: *"At this time there are no public APIs for preventing screen capture."*
- **Our own CI evidence agrees, and shows it is OS-version-dependent, not a one-off:** on real GitHub-hosted macOS hardware, `screencapture -V` (the same mechanism as ⇧⌘5 and QuickTime screen recording) captured the protected window on macOS 15.7.9 and did not on macOS 26.6.2 — same code, same commit, different result by OS version. Run [35136291136](https://github.com/jerryboganda/oetwebapp/actions/runs/35136291136), 16 Sep 2026.
- Electron hit the identical limitation and closed the equivalent issue as won't-fix: *"the only way to support this… is to enable some form of proper DRM for your window."*

**Owner decision (17 Sep 2026):** no macOS desktop build ships to candidates claiming "screenshot and screen-recording protection: PASSED" until it is proven so, on a phone-filmed physical Mac, on every macOS version the product officially supports. The recommended route is FairPlay Streaming DRM via Bunny MediaCage **Enterprise** (Basic has no screen-grab protection), played through the existing player where possible. Full pre-implementation research (Bunny pricing, Apple FPS approval process, WebView-vs-native-player analysis, a per-gate-item mapping, and a real-Mac test matrix) is recorded in project memory `oet-macos-video-079-0710-2026-09-16` pending owner approval; nothing has been implemented yet.

Until this is resolved: **do not advertise or support macOS for protected video playback**, and the live desktop build must not be represented as meeting this requirement.
