    ...
    <LearnerDashboardShell>
      <MotionSection className="max-w-3xl mx-auto space-y-6">
        ...
        <div className="flex items-center justify-center gap-2 mb-4">
          {TOUR_STEPS.map((s, i) => (
            <button key={s.id} onClick={() => setCurrentStep(i)} className={`w-3 h-3 rounded-full transition-[background-color,transform] duration-200 ${i === currentStep ? 'bg-primary scale-125' : completedSteps.has(i) ? 'bg-success' : 'bg-muted'}`} aria-label={`Step ${i + 1}: ${s.title}`} />
          ))}
        </div>
        ...
        <MotionItem key={step.id}>
          <Card className={`p-8 ${step.bg} border-2`}>
            <div className="text-center space-y-4">
              ...
              <div className="bg-surface/80 rounded-lg p-4 mt-4">
        ...
        <div className="text-center">
          <Button variant="ghost" size="sm" onClick={finishTour} className="text-muted">
