    ...
    <LearnerDashboardShell>
      ...
      <MotionSection className="space-y-5 sm:space-y-8 max-w-4xl mx-auto">
          ...
          {SCORING_GUIDE.map(s => (
            <MotionItem key={s.grade}>
              <Card className="p-4 flex items-center gap-3">
                ...
                <div><p className="font-semibold">{s.level} <span className="text-muted font-normal text-sm">({s.range})</span></p><p className="text-sm text-muted">{s.description}</p></div>
              </Card>
            </MotionItem>
          ))}
        </div>
        ...
        <Card className="p-5 space-y-2">
          ...
          <p className="text-sm">• <strong>Platform delivery scope</strong>: Website computer-based practice and OET@Home-style rehearsal only; paper-based exam behaviour is educational guidance, not a simulated platform mode</p>
          <p className="text-sm">• <strong>Practice results</strong>: deterministic raw and owner-table scores are shown after submission; they are not official OET results</p>
