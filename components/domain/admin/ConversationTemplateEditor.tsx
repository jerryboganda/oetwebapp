...
const EMPTY: TemplateForm = {
  ...
  expectedRedFlags: [''],
  keyVocabulary: [''],
  patientVoiceGender: 'female',
  patientVoiceAge: '45',
  patientVoiceAccent: 'en-GB',
  patientVoiceTone: 'neutral',
...
export function ConversationTemplateEditor({ templateId }: Props) {
  ...
  async function handleSave(): Promise<string | null> {
    ...
    const body = {
      ...
      patientVoice: {
        ...
        accent: form.patientVoiceAccent,
        tone: form.patientVoiceTone,
      },
    };
    try {
      if (templateId) {
  ...
  function updateListItem(
    field: 'objectives' | 'expectedRedFlags' | 'keyVocabulary',
    index: number,
    value: string,
  ) {
    setForm((prev) => ({
  ...
  if (loading) {
    return (
      <AdminDashboardShell>
        <AdminRouteWorkspace>
          <AdminRoutePanel>Loading…</AdminRoutePanel>
        </AdminRouteWorkspace>
      </AdminDashboardShell>
    );
  }
      ...
      <AdminRouteWorkspace>
        <AdminRoutePanel>
          <AdminRouteSectionHeader
            ...
            actions={
              <div className="flex items-center gap-2">
                <Badge
                  variant={
                    status === 'published' ? 'success' : status === 'archived' ? 'muted' : 'info'
                  }
                  size="sm"
                >
                  {status}
          ...
          <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
            ...
            <div>
              ...
              <select
                ...
                onChange={(e) =>
                  setForm((p) => ({
                    ...p,
                    taskTypeCode: e.target.value as 'oet-roleplay' | 'oet-handover',
                  }))
                }
            ...
            <div>
              ...
              <select
                ...
                onChange={(e) =>
                  setForm((p) => ({ ...p, difficulty: e.target.value as 'easy' | 'medium' | 'hard' }))
                }
                className="w-full rounded-lg border border-border bg-surface px-3 py-2 text-sm"
              >
            ...
            <Input
              ...
              type="number"
              value={String(form.estimatedDurationSeconds)}
              onChange={(e) =>
                setForm((p) => ({ ...p, estimatedDurationSeconds: Number(e.target.value) || 300 }))
              }
            />
          </div>
          ...
          <div className="mt-6 grid grid-cols-1 gap-6">
            <Textarea
              label="Scenario (candidate-facing context)"
              value={form.scenario}
            ...
            />
            <Textarea
              label="Patient / Colleague Context"
          ...
          <ListEditor
            label="Objectives (min 3 for publish gate)"
          ...
          <ListEditor
            label="Expected Red Flags"
            values={form.expectedRedFlags}
