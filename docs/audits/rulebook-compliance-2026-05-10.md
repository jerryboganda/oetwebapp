...
| # | Gap | Evidence | Fix | Resolution (2026-05-12) |
|---|---|---|---|---|
| P2-1 | `WritingCoachService` (live in-editor coach) is regex-stub only. Does not surface deterministic rule violations from `WritingRuleEngine`. | `backend/src/OetLearner.Api/Services/WritingCoachService.cs#L14-L100`. | Replace regex stubs by delegating to `WritingRuleEngine` for the live editor, returning a debounced subset (sentence-length, contractions, "the patient", Latin abbreviations). | ✅ **RESOLVED on `main` via `9d8003c`.** `WritingCoachService` rewritten (511-line change) to delegate to `WritingRuleEngine` for deterministic findings + AI suggestions through the gateway, deduped by `ruleId+anchor`, kill-switch honoured. Validated by `WritingCoachServiceTests` (16+ tests). |
| P2-2 | No first-class `rule_violations` table — violations buried in `Evaluation.FeedbackItemsJson`. Limits per-rule analytics ("which rule fails most often across cohort?"). | Schema in `backend/src/OetLearner.Api/Domain/Entities.cs#L349-L380`. | Add `WritingRuleViolation { Id, EvaluationId, RuleId, Severity, Quote, FixSuggestion, CreatedAt }`. Migration + index on `(RuleId, CreatedAt)`. | 🟡 **DEFERRED — not v1 launch blocker.** `Evaluation.FeedbackItemsJson` is queryable via JSON containment in PostgreSQL; cohort analytics via the existing admin Writing analytics endpoint. Tracked in `docs/audits/rulebook-compliance-2026-05-11-closure.md` as a follow-up wave for v1.1. |
...
| P3-3 | Writing rulebook drills only populated for the medicine profession. Other 12 professions have empty drill banks. | `rulebooks/writing/drills/medicine/**` only. | 🟡 **PARTIAL — schedule documented.** New `docs/audits/WRITING-DRILL-PROFESSION-COVERAGE.md` enumerates the 12 professions, the 6 drill types per profession, and the per-profession content-team owner. The shared `lib/writing-drills/loader.ts` validates each new profession's drills at module load (zod schema), so adding stubs is a content-only change with no app-side wiring. v1 launch ships with `medicine` only and a content roadmap for the rest. |
