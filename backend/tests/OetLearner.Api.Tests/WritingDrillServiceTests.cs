...
public class WritingDrillServiceTests
{
    ...
    public async Task SubmitCaseNoteDrill_ReturnsSentenceLevelFeedback()
    {
        ...
        var result = await service.SubmitCaseNoteDrillAsync(UserId, drill.Id, new OetLearner.Api.Services.Writing.WritingCaseNoteDrillAttemptRequest(responses, 45), CancellationToken.None);
