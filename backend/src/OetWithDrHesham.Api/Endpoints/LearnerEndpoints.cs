...
public static class LearnerEndpoints
{
    public static IEndpointRouteBuilder MapLearnerEndpoints(this IEndpointRouteBuilder app)
    {
        ...
        speaking.MapGet("/compliance", (
            Microsoft.Extensions.Options.IOptions<OetWithDrHesham.Api.Configuration.SpeakingComplianceOptions> opts) =>
        {
            var o = opts.Value;
            return Results.Ok(new
            {
        ...
        listening.MapPatch("/attempts/{attemptId}/heartbeat", async (HttpContext http, string attemptId, HeartbeatRequest request, LearnerService service, CancellationToken ct) => Results.Ok(await service.HeartbeatListeningAttemptAsync(http.UserId(), attemptId, request, ct)));
        ...
        v1.MapGet("/learner/learning-path", async (HttpContext http, [FromQuery] string? professionId, [FromQuery] string? examTypeCode, LearnerService service, CancellationToken ct)
            => Results.Ok(await service.GetLearningPathAsync(http.UserId(), professionId, OetWithDrHesham.Api.Services.Common.ExamCodes.NormalizeOrNull(examTypeCode) ?? OetWithDrHesham.Api.Services.Common.ExamCodes.DefaultCode, ct)))
            .RequireAuthorization("LearnerOnly");
