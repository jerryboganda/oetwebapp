...
- **Review Module (MISSION CRITICAL)**: All cross-skill retention runs through one engine. Every actionable mistake (Writing issues with severity ≥ medium, Speaking findings with severity ≥ medium, Reading/Listening/Mock misses, Pronunciation low-score phonemes, Grammar exercise errors) MUST create a `ReviewItem` via `IReviewItemSeeder` — **never** `db.ReviewItems.Add(...)` directly. Idempotency key is `(UserId, SourceType, SourceId)`; the `SourceType` enum is closed and lives in `ReviewSourceTypes`. SM-2 scheduling is delegated to the shared `ISpacedRepetitionScheduler`; never duplicate the algorithm. The `/v1/review/due` queue is **unified** — it merges native `ReviewItem` rows with a projection of due `LearnerVocabulary` cards (virtual ids `ri-v-{lvId}`); submission is routed transparently by `SpacedRepetitionService`. UI uses the shared 4-button quality scale (`Forgot/Hard/Good/Easy` → 0/2/3/5) and the polymorphic `ReviewItemRenderer` — never render raw `QuestionJson`/`AnswerJson`. Full spec: **[`docs/REVIEW-MODULE.md`](docs/REVIEW-MODULE.md)**.
