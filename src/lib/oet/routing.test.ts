...
describe("oet routing helpers", () => {
  it("maps each role to its primary landing route", () => {
    expect(resolveRoleLandingPath("learner")).toBe("/app/dashboard");
  ...
  it("infers the protected role surface from the pathname", () => {
    expect(getRoleFromPath("/app/writing/tasks")).toBe("learner");
  ...
  it("only grants access to the matching protected surface", () => {
    expect(canAccessRolePath("learner", "/app/dashboard")).toBe(true);
    ...
    expect(canAccessRolePath("admin", "/app/dashboard")).toBe(false);
  });
});
