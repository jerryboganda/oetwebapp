...
| BUG-004 | Critical | **OPEN** | Auto-updater ships pointing at `http://127.0.0.1:8765/latest.json` with a throwaway minisign key | Phase 0 | `tauri.conf.json:22-25`. Fix in Phase 3/8: production HTTPS feed + fresh key (private key in CI secret). |
...
| BUG-006 | Med | **OPEN** | Broad IPC ACL — all 17 commands granted to any `http://localhost*` / `127.0.0.1*` origin | Phase 0 | `remote-localhost.json`. Phase 3: confirm necessity (dynamic port), document residual risk, trim unused grants. |
