    ...
    <AdminOperationsLayout
      ...
      actions={
        <div className="flex items-center gap-2">
          <Link href="/admin/content/mocks" className="inline-flex items-center gap-1 text-sm text-admin-fg-muted hover:text-admin-fg-strong">
        ...
        <CardContent className="p-5 space-y-4">
          ...
          {loading ? (
            <div className="space-y-3">
              <Skeleton className="h-12 w-full rounded-admin" />
              <Skeleton className="h-12 w-full rounded-admin" />
              <Skeleton className="h-12 w-full rounded-admin" />
            </div>
          ) : items.length === 0 ? (
              ...
              <table className="min-w-full divide-y divide-admin-border text-sm">
                ...
                <tbody className="divide-y divide-admin-border">
                  {items.map((row) => {
                    ...
                    return (
                      <tr key={row.id} className="text-admin-fg-strong">
          ...
          {data?.generatedAt ? (
            <p className="text-xs text-admin-fg-muted">Last computed {new Date(data.generatedAt).toLocaleString()}.</p>
          ) : null}
        </CardContent>
      </Card>
    </AdminOperationsLayout>
