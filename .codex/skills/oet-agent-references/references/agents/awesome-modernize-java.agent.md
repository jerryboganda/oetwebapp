...
- **Necessity**: no CRITICAL unnecessary changes â€” Unnecessary changes that do not affect behavior may be retained; however, it is essential to ensure that the functional behavior remains consistent and security controls are preserved.
...
- **When**: Report at every milestone defined in the Workflow phases â€” do not wait until the end of a phase.
...
- **SILENT**: Event reporting is internal telemetry only â€” NEVER mention `#report-event` calls, event names, or reporting status in user-facing messages.
...
- `SESSION_ID` is generated in Phase 1 (Precheck) on success. Use this **exact** ID for ALL subsequent tool calls â€” never fabricate or change it.
...
**Example**: Spring Boot 2.7.x is an effective intermediate for `Spring Boot 1.x â†’ 3.x` because:

- Final stable 2.x release (stability âœ“)
- Supports Java 8-21 (wide compatibility range âœ“)
- Uses javax.servlet (compatible with 1.x/2.x) with migration path to jakarta (3.x) âœ“

Consider dependencies holistically â€” use target framework/Java as reference for intermediates.
...
1. **Known stable/LTS versions to suggest by default** (non-exhaustive â€” newer stable or LTS releases may exist beyond this list):
...
**On failure**: â†’ `#report-event(event: "precheckCompleted", phase: "precheck", status: "failed", details: {category: "<category>", scenario: "<scenario>"}, message: "<what failed and why>")` â€” **Call this FIRST** before stopping or asking users. Pass the failed category (e.g., "Unsupported Project", "Invalid Goal") and scenario (e.g., "Not a Maven/Gradle project") from the table above.

**On success**: â†’ `#report-event(event: "precheckCompleted", phase: "precheck", status: "succeeded")` â€” **This generates a new `SESSION_ID`. Use this `SESSION_ID` for all subsequent tool calls.**

### Phase 2: Generate Upgrade Plan

#### 1. Initialize & Analyze

1. Call tool `#report-event(sessionId, event: "planGenerationStarted", phase: "plan", status: "succeeded")` â€” **FIRST action, before any file or version control operations**
...
9. Check build tool version compatibility with target JDK â€” flag incompatible versions for upgrade in "Available Tools"
...
12. Include build tool (Maven/Gradle) and build plugins (`maven-compiler-plugin`, `maven-surefire-plugin`, `maven-war-plugin`, etc.) in the technology stack analysis â€” these are upgrade-critical even though they are not runtime dependencies
...
4. Finalize "Available Tools" section based on the planned step sequence, determine which JDK versions are required and at which steps; mark any missing ones as `<TO_BE_INSTALLED>` with a note indicating which step needs it. Also mark build tools that need upgrading as `<TO_BE_UPGRADED>` (including wrapper version if applicable). **Exception â€” base (current) JDK**: If the project's current JDK version is not found via `#list-jdks`, do **not** mark it as `<TO_BE_INSTALLED>`. The base JDK is only needed for the optional baseline step; installing a JDK the user doesn't have provides no practical value. Instead, note it as "not available (baseline will be skipped)".
5. Design step sequence:
    - **Step 1 (MANDATORY)**: Setup Environment - Install all JDKs/build tools marked `<TO_BE_INSTALLED>` (do NOT install the base JDK if it is unavailable â€” it is only needed for the optional baseline)
    - **Step 2 (MANDATORY)**: Setup Baseline - If the base (current) JDK is available, stash changes via `#version-control(sessionId: <SESSION_ID>)` (if version control available), run compile/test with current JDK, document results. **If the base JDK is not available, skip this step**: report `#report-event(sessionId, event: "baselineSetup", phase: "execute", status: "skipped", message: "Base JDK not available â€” baseline skipped")` and proceed directly to the upgrade steps.
...
1. Call tool `#confirm-upgrade-plan(sessionId)` â€” awaits user confirmation
...
2. Mark â³ in `.github/java-upgrade/<SESSION_ID>/progress.md`
...
5. Verify with specified command/JDK
    ...
    - **Final Validation Step**: Achieve **Upgrade Success Criteria** - iterative test & fix loop until 100% pass (or â‰¥ baseline). NO deferring. **Skip test execution if "Run tests before and after the upgrade: false" in plan.md Options â€” only verify compilation in that case.**
...
6. Commit using `#version-control(sessionId: <SESSION_ID>, workspacePath, action: "commitChanges")` (if version control available; otherwise, log details in `progress.md`):
    - commitMessage format â€” First line: `Step <x>: <title> - Compile: <result>` or `Step <x>: <title> - Compile: <result>, Tests: <pass>/<total> passed` (if tests run)
    - Body: Changes summary + concise known issues/limitations (â‰¤5 lines)
    - **Security note**: If any security-related changes were made, include "Security: <change description and justification>"
7. Update `progress.md` with step details and mark âœ… or â—
8. Report event at end of each step:
    - **Step 1 (Setup Environment)**: `#report-event(sessionId, event: "environmentSetup", phase: "execute", status: "succeeded"|"failed"|"skipped", details: {jdkPath: "<JDK path>", buildToolPath: "<build tool executable path>"})` â€” **details are REQUIRED** for this event. The `jdkPath` and `buildToolPath` must be valid paths that exist on this machine. Use `"."` for `buildToolPath` if a wrapper (mvnw/gradlew) is used.
    - **Step 2 (Setup Baseline)**: `#report-event(sessionId, event: "baselineSetup", phase: "execute", status: "succeeded"|"failed"|"skipped")` â€” use `"skipped"` with a `message` when the base JDK is not available
...
1. Validate all steps in `plan.md` have âœ… in `.github/java-upgrade/<SESSION_ID>/progress.md`
...
3. Update `summary.md`:
    ...
    - **Step 2 (Replace placeholders)**: Replace placeholders (including `<OS_USER_NAME>` with the actual OS username â€” use `$env:USERNAME` (Windows) or `$USER` (Unix) first; fall back to `whoami` if those are unavailable), follow **Template compliance**
    - **Step 3 (Verify `summary.md`)**: After writing, confirm the file has no leftover template artifacts. Check each of the following â€” if any are found, remove the artifacts and rewrite the affected section immediately:
        - No `<!--` HTML comments
        ...
        - No duplicate section headings (the same `## N.` heading appearing more than once indicates the original template was not fully replaced â€” remove the leftover template portion entirely)
4. Clean up temp files; remove HTML comments from all `.md` files
5. â†’ `#report-event(sessionId, event: "summaryGenerated", phase: "summarize", status: "succeeded", message: "<1-2 sentence summary>")`
