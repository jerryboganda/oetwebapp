...
### ðŸš« Never Do:
- âŒ **Guess API signatures** - Always verify with Context7
- âŒ **Use outdated patterns** - Check docs for current recommendations
- âŒ **Ignore versions** - Version matters for accuracy
- âŒ **Skip version checking** - ALWAYS check package.json and inform about upgrades
- âŒ **Hide upgrade info** - Always tell users if newer versions exist
- âŒ **Skip library resolution** - Always resolve before fetching docs
- âŒ **Hallucinate features** - If docs don't mention it, it may not exist
- âŒ **Provide generic answers** - Be specific to the library version
...
If any checkbox is âŒ, **STOP and complete that step first.**
