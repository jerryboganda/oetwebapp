...
public static class AdminEndpoints
{
    public static IEndpointRouteBuilder MapAdminEndpoints(this IEndpointRouteBuilder app)
    {
        ...
        admin.MapGet("/users/{userId}/access", async (string userId,
            OetWithDrHesham.Api.Services.Billing.UserAccessAllocationService svc, CancellationToken ct)
        ...
        admin.MapPost("/users/{userId}/hard-delete", async (
            ...
            OetWithDrHesham.Api.Services.Admin.UserHardDeleteService purge, LearnerDbContext db, CancellationToken ct) =>
        {
        ...
        admin.MapGet("/billing/provider-lifecycle-signals", async (AdminService service, CancellationToken ct,
            string? gateway, string? category, string? processingStatus, string? verificationStatus, string? search, int? page, int? pageSize)
            => Results.Ok(await service.GetBillingProviderLifecycleSignalsAsync(gateway, category, processingStatus, verificationStatus, search, page ?? 1, pageSize ?? 20, ct)))
            .WithAdminRead("AdminBillingRead");
        ...
        admin.MapGet("/grammar/lessons/{lessonId}/publish-gate",
            async (string lessonId,
                OetWithDrHesham.Api.Services.Grammar.IGrammarPublishGateService gate,
                CancellationToken ct)
        ...
        admin.MapPost("/grammar/ai-draft",
            async (HttpContext http,
                ...
                OetWithDrHesham.Api.Services.Grammar.IGrammarDraftService draft,
                CancellationToken ct) =>
            {
                ...
                try
                {
                    var result = await draft.GenerateAsync(new OetWithDrHesham.Api.Services.Grammar.GrammarDraftRequest(
                        ExamTypeCode: OetWithDrHesham.Api.Services.Common.ExamCodes.NormalizeOrNull(body.ExamTypeCode) ?? OetWithDrHesham.Api.Services.Common.ExamCodes.DefaultCode,
                ...
                catch (OetWithDrHesham.Api.Services.AiManagement.AiQuotaDeniedException qex)
                {
        ...
        admin.MapPost("/writing/ai-draft",
            async (HttpContext http,
                ...
                OetWithDrHesham.Api.Services.Writing.IWritingDraftService draft,
                CancellationToken ct) =>
            {
                if (body is null || string.IsNullOrWhiteSpace(body.Prompt))
                    return Results.BadRequest(new { error = "prompt is required" });
                ...
                try
                {
                    var result = await draft.GenerateAsync(new OetWithDrHesham.Api.Services.Writing.WritingDraftRequest(
        ...
        admin.MapGet("/writing/options",
            async (OetWithDrHesham.Api.Services.Writing.IWritingOptionsProvider provider,
                CancellationToken ct) =>
            {
        ...
        admin.MapPut("/writing/options",
            async (HttpContext http,
                ...
                OetWithDrHesham.Api.Services.Writing.IWritingOptionsProvider provider,
                CancellationToken ct) =>
            {
                var update = new OetWithDrHesham.Api.Domain.WritingOptions
                {
                    Id = "global",
        ...
        // backend/src/OetWithDrHesham.Api/Domain/RecallSetCodes.cs.
        ...
        admin.MapGet("/conversation/settings", async (
                OetWithDrHesham.Api.Services.Conversation.IConversationOptionsProvider provider,
                CancellationToken ct) =>
            {
        ...
        admin.MapPut("/conversation/settings", async (
                HttpContext http,
                OetWithDrHesham.Api.Contracts.AdminConversationSettingsRequest request,
                OetWithDrHesham.Api.Data.LearnerDbContext db,
                OetWithDrHesham.Api.Services.Conversation.IConversationOptionsProvider provider,
                ...
                CancellationToken ct) =>
            {
                ...
                if (row is null)
                {
                    row = new OetWithDrHesham.Api.Domain.ConversationSettingsRow { Id = "default", UpdatedAt = now };
                ...
                var optsProvider = (OetWithDrHesham.Api.Services.Conversation.ConversationOptionsProvider)provider;
        ...
        admin.MapGet("/conversation/sessions/{sessionId}", async (
                ...
                CancellationToken ct) =>
            {
                ...
                var annotations = evaluation is null
                    ? new List<OetWithDrHesham.Api.Domain.ConversationTurnAnnotation>()
        ...
        admin.MapPost("/conversation/tts-preview", async (
                OetWithDrHesham.Api.Contracts.AdminConversationTtsPreviewRequest request,
                OetWithDrHesham.Api.Services.Conversation.Tts.IConversationTtsProviderSelector selector,
                CancellationToken ct) =>
            {
                ...
                var result = await provider.SynthesizeAsync(
                    new OetWithDrHesham.Api.Services.Conversation.Tts.ConversationTtsRequest(
                        text, voice, locale,
        ...
        admin.MapPost("/pronunciation/drills/ai-draft", async (
            ...
            OetWithDrHesham.Api.Services.Pronunciation.IPronunciationAdminDraftService draftService,
            CancellationToken ct)
    ...
    private static void ValidateConversationSettingsRequest(AdminConversationSettingsRequest request)
    {
        ...
        if (request.RealtimeSttMaxChunkBytes is < 1024 or > OetWithDrHesham.Api.Services.Conversation.ConversationRealtimeTransportLimits.MaxBinaryChunkBytes)
        {
            throw ApiException.Validation("REALTIME_CHUNK_LIMIT", $"Realtime STT chunk size must be between 1024 and {OetWithDrHesham.Api.Services.Conversation.ConversationRealtimeTransportLimits.MaxBinaryChunkBytes} bytes.");
        }
