...
public static class WritingPathwayEndpoints
{
    ...
    private static void MapRoutes(RouteGroupBuilder group)
    {
        ...
        group.MapPost("/case-note-drills/{id:guid}/attempts", async (Guid id, OetWithDrHesham.Api.Contracts.WritingCaseNoteDrillAttemptRequest request, HttpContext http, IWritingCaseNoteDrillService service, CancellationToken ct) =>
        {
            var result = await service.SubmitAttemptAsync(http.UserId(), id, new OetWithDrHesham.Api.Services.Writing.WritingCaseNoteDrillAttemptRequest(request.Responses, request.TimeSpentSeconds), ct);
