...
| macOS desktop **auto-update is not possible** | `createUpdaterArtifacts: true` is set, but `bundle.targets` is `["nsis","dmg"]` and the updater needs the `app` target on macOS — the build warns and produces no `.app.tar.gz`. | The published desktop feed has no macOS entry (Windows only). Fix by adding `"app"` to the targets **together with** signing: an unsigned macOS self-update can be refused by Gatekeeper. |
