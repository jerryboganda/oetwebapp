...
| `security.deviceChangeWindowDays` / `security.deviceChangeMaxPerWindow` | `7` / `3` | Device-change cooldown: more than N OTP-approved replacement identities inside the window blocks further changes (`device_change_cooldown`). The initial bootstrap is not counted. | Raise if support sees legitimate multi-device users hitting the cooldown. |
