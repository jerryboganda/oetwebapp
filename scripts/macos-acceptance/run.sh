...
# PASS / FAIL / NOT_AUTOMATED / INFO; the script exits non-zero on any FAIL.
#
...
capture_battery() { # capture_battery <tag> [record-seconds]
  local tag="$1" seconds="${2:-0}"
  ...
  for png in "$OUT/$tag-screencapture.png" "$OUT/$tag-screencapturekit.png" ${frames[@]+"${frames[@]}"}; do
    ...
    local json verdict
    json="$("$TOOL" classify "$png" "$OUT/$tag-window.json" "$OUT/controls.json" 2>> "$OUT/commands.log")"
...
ERRORS="~Could not start playback"
player_ready() { # player_ready <n>
  ...
  if "$TOOL" wait-element Fullscreen 90 >> "$OUT/commands.log" 2>&1 \
    ...
    for text in "$ERRORS" "~Secure display protection is unavailable" "~could not be verified"; do
      "$TOOL" wait-element "$text" 1 >> "$OUT/commands.log" 2>&1 && { record "video $n player" FAIL "error panel: $text"; return 1; }
    done
    record "video $n player" PASS "protected Bunny player loaded in the app window (no 403 / error panel)"
  else
...
if player_ready 1; then
  capture_battery v1-loaded
  toggle_play
  if wait_playing; then record "video 1 play" PASS "media playback assertion active"; else record "video 1 play" FAIL "no playback after Space"; fi
  sleep 20
  capture_battery v1-playing 20
  toggle_play
...
open "oet-prep://videos"
check "return to library" "library page shown" "$TOOL" wait-element "~Video Library" 30
open "oet-prep://videos/${VIDS[1]}"
SEEKED=false
if player_ready 2; then
  toggle_play
  wait_playing && record "video 2 play" PASS "media playback assertion active" || record "video 2 play" FAIL "no playback after Space"
  sleep 15
...
if player_ready 3; then
  toggle_play
  wait_playing && record "video 3 play" PASS "media playback assertion active" || record "video 3 play" FAIL "no playback after Space"
  ...
  capture_battery v3-fullscreen 15
...
# ── Summary ──────────────────────────────────────────────────────────────────
{
  echo "### macOS acceptance — $(sw_vers -productVersion), app $(defaults read "$APP/Contents/Info" CFBundleShortVersionString)"
  echo
...
awk -F'\t' '$2 == "FAIL" { failed = 1 } END { exit failed }' "$RESULTS"
