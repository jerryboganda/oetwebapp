...
/// per-page Markdown that preserves headings, bullets, and layout — the input
/// the Listening Part A extraction then hands to Claude for structuring.
///
...
{
    /// <summary>OCR a document and return the concatenated per-page Markdown.</summary>
    Task<string> OcrToMarkdownAsync(byte[] documentBytes, string mimeType, CancellationToken ct);
}
    ...
    IAiProviderRegistry registry) : IMistralOcrClient
{
    ...
    public async Task<string> OcrToMarkdownAsync(byte[] documentBytes, string mimeType, CancellationToken ct)
    {
        ...
        var apiKey = await registry.GetPlatformKeyAsync(ProviderCode, ct)
            ?? throw new InvalidOperationException($"Platform API key missing for provider {ProviderCode}.");
