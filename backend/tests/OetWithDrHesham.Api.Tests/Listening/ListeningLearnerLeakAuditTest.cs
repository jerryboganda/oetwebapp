...
public class ListeningLearnerLeakAuditTest
{
    ...
    private static string LocateListeningServicesDir()
    {
        ...
        for (int i = 0; i < 8; i++)
        {
            var candidate = Path.Combine(dir, "backend", "src", "OetWithDrHesham.Api", "Services", "Listening");
            if (Directory.Exists(candidate)) return candidate;
            dir = Path.GetDirectoryName(dir) ?? dir;
        }
        throw new DirectoryNotFoundException(
            "Could not locate backend/src/OetWithDrHesham.Api/Services/Listening from test bin dir.");
    }
}
