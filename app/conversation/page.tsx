...
export default function ConversationPage() {
  ...
  const entitlementTone: 'default' | 'success' | 'warning' | 'danger' =
    !entitlement ? 'default'
      : !entitlement.allowed ? 'danger'
      : entitlement.tier === 'paid' || entitlement.tier === 'trial' ? 'success'
      : entitlement.remaining <= 1 ? 'warning'
      : 'default';
      ...
      <div className="space-y-6">
        ...
        {entitlement && !entitlement.allowed && (
          <InlineAlert variant="warning">
            ...
            {entitlement.resetAt && (
              <>
                {' '}Quota resets at <strong>{new Date(entitlement.resetAt).toLocaleString()}</strong>.
              </>
            )}
          </InlineAlert>
        )}
                    ...
                    >
                      <div className="flex items-start gap-3">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          {isCreatingThis && (
                            <p className="mt-2 text-xs text-primary">Creating session…</p>
                          )}
                        </div>
