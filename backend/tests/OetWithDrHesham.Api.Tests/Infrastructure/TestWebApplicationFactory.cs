...
namespace OetWithDrHesham.Api.Tests.Infrastructure;

public class TestWebApplicationFactory : WebApplicationFactory<Program>
{
    private readonly string _storageRoot = Path.Combine(Path.GetTempPath(), $"oet-with-dr-hesham-tests-storage-{Guid.NewGuid():N}");
    private readonly string _databaseName = $"oet-with-dr-hesham-tests-{Guid.NewGuid():N}";
    ...
    protected TestWebApplicationFactory(bool useFirstPartyAuth, bool seedDemoData)
    {
        ...
        _firstPartyConfiguration = new Dictionary<string, string?>
        {
            ...
            [$"{AuthTokenOptions.SectionName}:Audience"] = "oet-with-dr-hesham-web",
            ...
            [$"{AuthTokenOptions.SectionName}:AuthenticatorIssuer"] = "OET with Dr Hesham"
        };
    ...
    protected override void ConfigureWebHost(IWebHostBuilder builder)
    {
        ...
        builder.ConfigureTestServices(services =>
        {
            ...
            for (var i = services.Count - 1; i >= 0; i--)
            {
                if (services[i].ServiceType == typeof(OetWithDrHesham.Api.Services.Rulebook.IAiModelProvider))
                {
                    services.RemoveAt(i);
                }
            }
    ...
    /// <see cref="OetWithDrHesham.Api.Services.Billing.Oet2026CatalogSeeder"/>
    ...
    public async Task EnsureCatalogSeededAsync(CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            var seeder = scope.ServiceProvider
                .GetRequiredService<OetWithDrHesham.Api.Services.Billing.Oet2026CatalogSeeder>();
