...
name: "Awesome clojure-interactive-programming"
...
- Untestable functions requiring mocks
  â†’ **Action**: Flag violation, propose refactoring, fix root cause
...
- âœ… Config fails â†’ Show clear error message
- âœ… Service init fails â†’ Explicit error with missing component
- âŒ `(or server-config hardcoded-fallback)` â†’ Hides endpoint issues
...
**\"It works\" â‰  \"It's done\"** - Working means functional, Done means quality criteria met.
