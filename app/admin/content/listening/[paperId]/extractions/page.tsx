  ...
  return (
    ...
    >
      <div className="space-y-4">
        ...
        <Textarea
          label={mode === 'reject' ? 'Reason (required)' : 'Note (optional)'}
          value={reason}
...
function DraftCard({ draft, paperId: _paperId, onApprove, onReject }: DraftCardProps) {
  ...
  return (
    <Card className="space-y-4">
      ...
      {/* Summary */}
      {draft.summary && (
        <p className="text-sm text-navy">{draft.summary}</p>
      )}

      {/* Decision info for non-pending */}
      {!isPending && draft.decidedAt && (
        <div className="rounded-xl bg-background-light p-3 text-xs text-muted space-y-0.5">
          <p>
          ...
          {expanded && (
            <div className="mt-3 overflow-x-auto rounded-xl border border-border">
              <table className="w-full min-w-[600px] text-left text-sm">
                <thead>
                  <tr className="border-b border-border bg-background-light text-xs font-semibold uppercase tracking-widest text-muted">
                ...
                <tbody className="divide-y divide-border px-3">
                  {draft.questions.map((q) => (
                    <tr key={q.id ?? q.number} className="border-b border-border last:border-0">
                      <td className="py-2 pl-3 pr-3 font-mono text-xs text-muted">{q.number}</td>
                      ...
                      <td className="max-w-xs py-2 pr-3 text-sm text-navy">
                        <span className="line-clamp-2">{q.stem || <em className="text-muted">empty</em>}</span>
                      </td>
...
export default function AdminListeningExtractionsPage() {
  ...
  return (
    <AdminRouteWorkspace role="main" aria-label="Listening extraction drafts">
      <AdminRouteSectionHeader
      ...
      {STATUS_TABS.map((tab) => (
        <TabPanel key={tab.id} id={tab.id} activeTab={activeTab}>
          <AdminRoutePanel title={`${tab.label} Drafts`}>
