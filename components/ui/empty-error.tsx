...
export function ErrorState({ title = 'Something went wrong', message = 'An unexpected error occurred. Please try again.', onRetry, className }: ErrorStateProps) {
  ...
  return (
    <motion.div
      ...
      {...motionProps}
      className={cn(
        'flex flex-col items-center justify-center rounded-2xl border border-red-100 bg-red-50/70 px-6 py-12 text-center shadow-sm dark:border-red-900 dark:bg-red-950/70',
        className,
      )}
    >
