...
- **Grammar Module (MISSION CRITICAL)**: All grammar logic (topic discovery, lesson delivery, exercise grading, mastery tracking, recommendation mapping) MUST route through `lib/grammar/*` (TS) and `OetLearner.Api.Services.Grammar.*` (.NET). Grading is **server-authoritative** via `GrammarGradingService`; learner endpoints use separate DTOs (`GrammarLessonLearnerDto`, `GrammarExerciseLearnerDto`) that **never** serialise `correctAnswerJson` / `acceptedAnswersJson` / `explanationMarkdown` until after submission. AI draft generation goes through `AiGatewayService.BuildGroundedPrompt` with `FeatureCode=grammar_draft`. The publish gate requires `TopicId`, ≥ 1 content block, ≥ 3 exercises, non-empty `SourceProvenance`. Wrong answers create `ReviewItem` rows (`SourceType="grammar_error"`); writing/speaking rule flags create `GrammarRecommendation` rows. Full spec: **[`docs/GRAMMAR.md`](docs/GRAMMAR.md)**. Configurable policy: **[`docs/GRAMMAR-POLICY.md`](docs/GRAMMAR-POLICY.md)**. Never render admin-authored grammar content via `dangerouslySetInnerHTML`; always use `SafeRichText` / `GrammarContentRenderer`.
