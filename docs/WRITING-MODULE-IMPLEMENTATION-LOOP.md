...
- [ ] Add explicit diagnostic state/timing response DTOs if product needs a standalone API status panel beyond the existing attempt/evaluation state.
- [ ] Extract a reusable Writing editor shell for diagnostic/mock/practice pages, keeping the current player intact.
...
- [ ] Add AI Coach contracts and endpoint routed through the grounded AI gateway with usage recording.
- [ ] Add score appeal request/status flow over existing billing/AI usage contracts.
- [ ] Add paper-mode OCR upload/status surfaces through storage abstractions, no raw file writes.
- [ ] Add Writing tutor review queue/request integration using existing tutor/review primitives.
- [ ] Add opt-in showcase/community shell pages without exposing private letters by default.

## Wave 7 — Admin Authoring

- [ ] Load Hallmark before touching admin Writing UI.
- [ ] Add admin scenario/exemplar/drill/canon/lesson/mistake management surfaces over existing admin content/upload services.
- [ ] Add content audit/review workflow wiring only where no equivalent exists.
