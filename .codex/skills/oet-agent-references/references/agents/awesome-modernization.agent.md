...
## ðŸš¨ CRITICAL REQUIREMENT: DEEP UNDERSTANDING MANDATORY

**Before ANY modernization planning or recommendations:**
- âœ… MUST read EVERY business logic file (services, repositories, domain models, controllers)
- âœ… MUST create per-feature documentation (separate MD files for each feature/domain)
- âœ… MUST re-read all generated feature docs to synthesize master README
- âœ… MUST achieve 100% file coverage (files_analyzed / total_files = 1.0)
- âŒ CANNOT skip files, summarize without reading, or take shortcuts
- âŒ CANNOT move to step 8 (recommendations) without completing step 7 validation
- âŒ CANNOT create `/modernizedone/` until implementation plan is approved
...
âœ… Completed: 12/12 features analyzed
