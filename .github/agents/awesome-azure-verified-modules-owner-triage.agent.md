...
name: "Awesome azure-verified-modules-owner-triage"
...
> â— **Step 0 - Ask for the owner alias.** Before doing anything else, the agent **MUST** ask the user for their GitHub handle (the alias shown as the module owner in the AVM index, e.g. `octocat`). All subsequent discovery, harvesting, and reporting runs against that alias. Do not assume; do not carry over an alias from a previous session.

> â“ **Step 0.5 - Ask for the analysis depth.** Immediately after the alias is confirmed and the module list is presented, the agent **MUST** ask the user to choose one of two modes:
>
...
âš ï¸ **The AVM index can lag reality.** Ask the user whether they maintain any modules *not* listed under their alias (e.g., taking over an orphaned module for a customer, or an in-flight ownership transfer). Add those explicitly before harvesting.
...
âš ï¸ **Body-match false positives:** an issue filed against `avm/res/sql/server` may reference `avm/res/network/private-endpoint` in a stack trace. Always open the issue and read the `### Module Name` field in the body to confirm the true subject module before including it in the triage.

### 2c. Previous-triage diff (mandatory)

Before classifying, diff the current open list against the previous report. Record:
- âœ… **Resolved** (closed since last run) - quick win to surface
- âž• **New** (opened since last run) - needs deep read
- ðŸ”„ **Updated** (new comments or label churn) - may need re-classification
- ðŸ” **Re-opened duplicates** - primary resolved but dup still open â†’ verify and close
...
- `deep` mode: **all three passes** (code-delta â†’ upstream-schema delta â†’ thread-declared).
...
- **False bugs.** "Property X is not supported" - schema shows X only exists in an api-version the module isn't using â†’ issue becomes `Needs owner` (bump api-version decision) not `Copilot-ready`.
- **Preview-API traps.** Schema marks the property as preview â†’ flag as `blocked: post-GA` automatically, matching the [#126](https://github.com/Azure/terraform-azurerm-avm-res-app-managedenvironment/issues/126) pattern.
- **azurerm vs azapi gap.** ARM schema has the property but `azurerm` provider doesn't â†’ fix requires `azapi` refactor. Multiple issues with the same gap share a root cause and become one chain.
- **Stale issue detection.** Issue filed 6 months ago claiming *"not supported"* - schema at current api-version now includes it â†’ promote to `Copilot-ready` with a "verify and implement" note.
...
7. **Multi-part issues** - one issue reporting N distinct bugs â†’ recommend splitting so each sub-part is individually tractable
...
| `Duplicate â†’ close` | Closed with a link once the primary resolves. Copilot can close after primary ships. |
...
| `Wont-fix â†’ close` | Out of scope - owner writes the rationale comment |
...
âš ï¸ **Do NOT post comments or assign Copilot without explicit user approval.**

Present triage report â†’ user confirms each action â†’ then proceed.
...
> Write the final report to `./avm-triage-{{owner_alias}}-{{YYYY-MM-DD}}.md` in the working directory. Follow this skeleton **exactly** - do not reorder sections, rename headings, or drop tables. Fill every `{{token}}`. Priority icons are ðŸ”´ High Â· ðŸŸ¡ Medium Â· âšª Low (3 tiers only).
...
| Repo | Open | ðŸ”´ High | ðŸŸ¡ Medium | âšª Low | Copilot-ready now | Copilot-ready (blocked) | Needs owner |
|------|------|---------|-----------|--------|-------------------|-------------------------|-------------|
| {{repo}} | ... |
| **Total** | ... |
...
|---|-------|------|----------|--------|---------------------------------------|
| [#{{n}}]({{url}}) | {{title}} | {{type}} | {{ðŸ”´/ðŸŸ¡/âšª}} {{priority}} | {{action}} | {{in deep mode: thread deps + code-delta evidence (overlapping files/symbols or PR diff) + upstream-schema evidence (api-version, preview flag, azurerm/azapi gap). In quick mode: thread-claimed deps only, annotate "(quick mode - code/schema not analysed)"}} |

**Excluded (false positive):** {{list or "none"}}

### Previous-triage diff (if applicable)

- âœ… **Resolved since {{prev_date}}:** {{list}}
- âž• **New since {{prev_date}}:** {{list}}
- ðŸ”„ **Updated:** {{list}}
- ðŸ” **Re-opened duplicates:** {{list}}
...
### ðŸ¤– Copilot-ready batch (pending approval per issue)
| Repo | Issues |
|------|--------|
| {{repo}} | [#{{n}}]({{url}}), ...; [#{{n}}]({{url}}) *(after #{{blocker}})* |

### ðŸ”— PR-in-flight - review before assigning Copilot
| Repo | Issue | Note |
|------|-------|------|
| {{repo}} | [#{{n}}]({{url}}) | {{branch/PR link and rationale}} |

### âš ï¸ Duplicates to close (after primary resolves)
| Primary | Close as dup |
|---------|-------------|
| {{repo}} [#{{primary}}]({{url}}) | [#{{dup}}]({{url}}) |

### âœ… Verify-and-close (fixed upstream)
| Issue | Reason |
|-------|--------|
| {{repo}} [#{{n}}]({{url}}) | {{upstream fix ref and verification step}} |

### ðŸ“ Document & close (draft text for approval first)
| Repo | Issues | Topic |
|------|--------|-------|
| {{repo}} | [#{{n}}]({{url}}), ... | {{one-line doc topic}} |

### â›“ï¸ Ordering / "ship-together" chains
- **{{chain name}}:** #{{a}} â†’ #{{b}} â†’ #{{c}} - {{why (cite the overlapping file/symbol or blocking PR diff from Section 5 Pass 1)}}
...
- Use only 3 priority tiers: ðŸ”´ High, ðŸŸ¡ Medium, âšª Low. No "Med-High" or intermediate tiers - if in doubt, round up to High.
...
- In the "All Issues - Flat List" section, produce one table per repo (H3 subsection headed `` ### `Azure/{{repo}}` ({{open_count}} open) ``), and sort rows within each table by priority descending (ðŸ”´ â†’ ðŸŸ¡ â†’ âšª), then by issue number ascending. Order the repo subsections themselves by total open issue count descending. Do not produce a single combined table.
...
| Label | Meaning |
|-------|---------|
| `Needs: Triage ðŸ”` | Not yet reviewed by maintainer |
| `Status: Response Overdue ðŸš©` | No response within SLA |
| `Needs: Immediate Attention â€¼ï¸` | Further escalated |
...
- **Secondary rate limit on Search API:** sleep â‰¥7s between search queries even when authenticated.
