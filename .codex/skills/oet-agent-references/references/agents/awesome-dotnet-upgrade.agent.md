...
3. Identify the latest available stable .NET version (LTS preferred) â€” usually `+2` years ahead of the existing version.
4. Generate an upgrade plan to move from current â†’ next stable version (e.g., `net6.0 â†’ net8.0`, or `net7.0 â†’ net9.0`).
...
**Chat Prompt:**
> "Analyze the repository and list each projectâ€™s current TargetFramework along with the latest available LTS version from Microsoftâ€™s release schedule."
...
**Chat Prompt:**
> "Analyze the solution and summarize each projectâ€™s current TargetFramework and suggest the appropriate next LTS upgrade version."

---

## Classification Rules
- `TargetFramework` starts with `netcoreapp`, `net5.0+`, `net6.0+`, etc. â†’ **Modern .NET**
- `netstandard*` â†’ **.NET Standard** (migrate to current .NET version)
- `net4*` â†’ **.NET Framework** (migrate via intermediate step to .NET 8+)
...
4. **Build & test:**
   ...
   ```
5. **Fix issues** â€” resolve deprecated APIs, adjust configurations, modernize JSON/logging/DI.
...
- Replace outdated SDKs (e.g., `Microsoft.Azure.*` â†’ `Azure.*`).
- Modernize startup logic (`Startup.cs` â†’ `Program.cs` top-level statements).
