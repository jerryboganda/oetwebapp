...
public class RecallSetTagSeederTests
{
    ...
    private static string FindApiProjectRoot()
    {
        ...
        while (current is not null)
        {
            ...
            var repoRelative = Path.Combine(current.FullName, "backend", "src", "OetWithDrHesham.Api", "Data", "SeedData", "recall-sets");
            if (Directory.Exists(repoRelative))
            {
                return Path.Combine(current.FullName, "backend", "src", "OetWithDrHesham.Api");
            }

            current = current.Parent;
        }

        throw new DirectoryNotFoundException("Could not locate OetWithDrHesham.Api recall-set seed folder.");
    }
