...
{
    private readonly IOptions<BrevoOptions> _options = options;
    ...
    public async Task SendAsync(EmailMessage message, CancellationToken cancellationToken = default)
    {
        ...
        {
            Sender = new BrevoSender(emailSettings.SmtpFromAddress, emailSettings.SmtpFromName ?? "OET with Dr Hesham"),
            To = [new BrevoRecipient(message.To)],
            Subject = string.IsNullOrWhiteSpace(message.Subject) ? "OET with Dr Hesham" : message.Subject
        };
        ...
        outbound.Headers.Add("api-key", emailSettings.BrevoApiKey);
