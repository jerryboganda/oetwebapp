---
name: "Awesome react18-batching-fixer"
...
this.setState({ loading: true });     // â†’ React re-renders immediately
// ... re-render happened, this.state.loading === true
const data = await fetchData();
if (this.state.loading) {             // â† reads the UPDATED state
...
this.setState({ loading: true });     // â†’ BATCHED - no immediate re-render
// ... NO re-render yet, this.state.loading is STILL false
const data = await fetchData();
if (this.state.loading) {             // â† STILL false! The condition fails silently.
  this.setState({ data, loading: false }); // â† never called
}
...
async loadUser() {
  ...
  if (this.state.loading) {           // â† BUG: loading never true here in React 18
...
it('shows loading state', async () => {
  ...
  expect(screen.getByText('Loading...')).toBeInTheDocument(); // â† may not render yet in React 18
