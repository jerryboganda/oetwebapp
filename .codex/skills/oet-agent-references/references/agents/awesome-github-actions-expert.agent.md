---
name: "Awesome github-actions-expert"
...
- **Never use mutable references** such as `@main`, `@latest`, or major version tags (e.g., `@v4`) â€” tags can be silently moved by a repository owner or attacker to point to a malicious commit, enabling supply chain attacks that execute arbitrary code in your CI/CD pipeline
...
1. Pin actions to full commit SHAs with version comments (e.g., `@<sha> # vX.Y.Z`) â€” never use mutable tags or branches
