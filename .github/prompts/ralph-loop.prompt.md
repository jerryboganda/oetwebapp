...
Read `AGENTS.md`, `.github/agent-state.local.md` if present, compact `PROGRESS.md`, and then only the relevant `PRD.md` sections. Treat `AGENTS.md` as higher priority for fast-feedback/host validation rules. Execute one coherent task slice at a time, review it, update `.github/agent-state.local.md` and `PROGRESS.md` when appropriate, and continue until complete or blocked.