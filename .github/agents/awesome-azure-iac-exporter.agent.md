---
name: "Awesome azure-iac-exporter"
...
- Use `#tool:execute` with `az rest` commands to collect only user-configured data plane properties:
  - **Storage Accounts**: `az rest --method GET --url "https://management.azure.com/{storageAccountId}/blobServices/default?api-version=2023-01-01"` â†’ Filter for user-set CORS, lifecycle policies, encryption settings
  - **Key Vault**: `az rest --method GET --url "https://management.azure.com/{keyVaultId}?api-version=2023-07-01"` â†’ Filter for custom access policies, network rules
  - **App Service**: `az rest --method GET --url "https://management.azure.com/{appServiceId}/config/appsettings/list?api-version=2023-01-01"` â†’ Extract custom application settings only
  - **AKS**: `az rest --method GET --url "https://management.azure.com/{aksId}/agentPools?api-version=2023-10-01"` â†’ Filter for custom node pool configurations
  - **Cosmos DB**: `az rest --method GET --url "https://management.azure.com/{cosmosDbId}/sqlDatabases?api-version=2023-11-15"` â†’ Extract custom consistency, indexing policies
...
- âœ… Syntactically valid IaC templates in the user's chosen format
- âœ… Schema-compliant resource definitions with latest API versions (especially for Bicep)
- âœ… Deployable parameter/variable files
- âœ… Comprehensive storage account configuration including dataplane settings
- âœ… Clear deployment documentation and usage instructions
- âœ… Meaningful parameter descriptions and validation rules
- âœ… Ready-to-use deployment artifacts
