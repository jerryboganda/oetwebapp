...
## PHASE 7 - Event Delegation Change (React 16 â†’ 17 Carry-Over)
...
- `@testing-library/react` â†’ 14+ (RTL 13 uses `ReactDOM.render` internally)
- `@apollo/client` â†’ 3.8+ for React 18 concurrent mode support
- `@emotion/react` â†’ 11.10+ for React 18
- `react-router-dom` â†’ v6.x for React 18
...
**Critical:** If Enzyme is found â†’ this is a major blocker. Enzyme does not support React 18. Every Enzyme test must be rewritten using React Testing Library.
...
## ðŸ”´ Critical - Silent Runtime Breakers
...
## ðŸŸ  Unsafe Lifecycle Methods (Warns in 18.3.1, Required for React 19)

### componentWillMount (â†’ componentDidMount or constructor)
...
### ReactDOM.render (â†’ createRoot - required for batching)
[List all hits]

## ðŸŸ¡ Deprecated APIs (Warn in 18.3.1, Removed in React 19)
...
5. Migrate componentWillMount â†’ componentDidMount
6. Migrate componentWillReceiveProps â†’ getDerivedStateFromProps/componentDidUpdate
7. Migrate componentWillUpdate â†’ getSnapshotBeforeUpdate/componentDidUpdate
8. Migrate Legacy Context â†’ createContext
9. Migrate String Refs â†’ React.createRef()
10. Remove findDOMNode â†’ direct refs
11. Migrate ReactDOM.render â†’ createRoot
12. Audit all async setState chains - add flushSync where needed
13. Review document.addEventListener patterns
14. Run full test suite â†’ fix failures
