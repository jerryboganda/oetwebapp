...
public static class MediaEndpoints
{
        ...
        CancellationToken ct)
    {
        ...
        var isInUse = await db.ContentPaperAssets.AnyAsync(link => link.MediaAssetId == asset.Id, ct)
            ...
            || await db.WritingScenarios.AnyAsync(s => s.StimulusPdfMediaAssetId == asset.Id, ct)
            || await db.WritingScenarios.AnyAsync(s => s.AnswerSheetPdfMediaAssetId == asset.Id, ct);
        if (isInUse)
        {
            return Results.Json(
