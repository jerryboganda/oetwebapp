 ...
 *   queryKeys.readiness.byFamily(f) // ['readiness', 'family', f]
 *
 * Invalidation:
...
/**
 * Example migrated hooks. Each follows the same three-line pattern:
 *   1. queryKey from `queryKeys.*`
 *   2. queryFn from `lib/api.ts` (source of truth for fetching)
 *   3. sensible staleTime override per resource
 *
...
export function useUserProfile(options: QueryOpts<Awaited<ReturnType<typeof fetchUserProfile>>> = {}) {
  return useQuery({
    ...
    staleTime: 60_000, // profile rarely changes mid-session
    ...options,
  });
}
...
/** Re-export the client accessor so callers don't need a second import. */
