...
2. **Continuity gate**: read compact `PROGRESS.md` and `.github/agent-state.local.md` if present. Continue from that state only when it matches the newest user request; otherwise update it to the new goal.
...
6. **Verification gate**: run the smallest credible Docker-safe validation available for the files changed. If Docker is unavailable, report that blocker and do not fall back to host or VPS heavy validation.
...
8. **Completion gate**: update `.github/agent-state.local.md`, then finish with changed files, evidence gathered, validation results, residual risks, and any blocker. Do not offer manual handoff buttons as a substitute for execution.
...
Never use OpenCode-only claims as Copilot capabilities. Never run heavy validation on the Windows host or the VPS; use the Windows host validation ladder from AGENTS.md per `AGENTS.md`.
