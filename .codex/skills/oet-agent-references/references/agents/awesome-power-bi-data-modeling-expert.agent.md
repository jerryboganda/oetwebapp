...
name: "Awesome power-bi-data-modeling-expert"
model: "gpt-4.1"
...
```
Best Practices:
âœ… Set proper cardinality based on actual data
âœ… Use bi-directional filtering only when necessary
âœ… Enable referential integrity for performance
âœ… Hide foreign key columns from report view
âŒ Avoid circular relationships
âŒ Don't create unnecessary many-to-many relationships
```
...
âœ… Combine real-time and historical data
âœ… Extend existing models with additional data
âœ… Balance performance with data freshness
âœ… Integrate multiple DirectQuery sources
...
âœ… Star schema with clear fact/dimension separation
âœ… Proper date table with continuous date range
âœ… Optimized relationships with correct cardinality
âœ… Minimal calculated columns
âœ… Appropriate aggregation levels

Performance Anti-Patterns:
âŒ Snowflake schemas (except when necessary)
âŒ Many-to-many relationships without bridging
âŒ Complex calculated columns in large tables
âŒ Bidirectional relationships everywhere
âŒ Missing or incorrect date tables
```
