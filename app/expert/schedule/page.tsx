...
import { fetchExpertSchedule, saveExpertSchedule } from '@/lib/api';
...
export default function SchedulePage() {
  ...
  const updateDay = (day: string, patch: Partial<ExpertScheduleDay>) => {
    ...
    if (validationErrors[day]) {
      setValidationErrors(prev => { const next = { ...prev }; delete next[day]; return next; });
    }
  };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setToast({ variant: 'error', message: 'Failed to save schedule.' });
    } finally {
      setIsSaving(false);
    }
  };
    ...
    <div className="max-w-4xl mx-auto p-4 md:p-8 space-y-6" role="main" aria-label="Schedule Management">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted text-sm mt-1">Manage your regular review working hours.</p>
        </div>
      </div>
          ...
          <div className="p-4 border-b border-gray-200 flex items-center justify-between flex-wrap gap-3">
            ...
            <div className="w-64">
              <Select
                ...
                onChange={(e) => schedule && setSchedule({ ...schedule, timezone: e.target.value })}
