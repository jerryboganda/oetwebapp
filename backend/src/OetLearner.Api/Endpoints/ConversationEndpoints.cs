...
public static class ConversationEndpoints
{
    public static IEndpointRouteBuilder MapConversationEndpoints(this IEndpointRouteBuilder app)
    {
        ...
        // Task type catalog (public to learners — tells UI which task types are enabled).
        conv.MapGet("/task-types", (ConversationService svc) =>
            Results.Ok(svc.GetTaskTypeCatalog()));

        // Entitlement — check before showing the "Start" button.
        ...
        // Audio media stream (content-addressed SHA-256). Authenticated learners only.
