...
public sealed class LearnerServicePerformanceTests : IAsyncLifetime
{
    ...
    private static LearnerService CreateLearnerService(LearnerDbContext db)
    {
        ...
        var platformLinks = new PlatformLinkService(
            ...
            billingOptions);
        var storageRoot = Path.Combine(Path.GetTempPath(), $"oet-with-dr-hesham-performance-{Guid.NewGuid():N}");
        ...
        IOptions<BillingOptions> billingOptions)
    {
        ...
        return new PaymentGatewayService(
            ...
            new OetWithDrHesham.Api.Services.Billing.Gateways.PayTabsGateway(
                new HttpClient(),
                billingOptions,
                runtimeSettings),
            new OetWithDrHesham.Api.Services.Billing.Gateways.PaymobGateway(
                new HttpClient(),
                billingOptions,
                runtimeSettings),
            new OetWithDrHesham.Api.Services.Billing.Gateways.CheckoutComGateway(
                new HttpClient(),
                billingOptions,
                runtimeSettings),
            new OetWithDrHesham.Api.Services.Billing.Gateways.EasyKashGateway(
                new HttpClient(),
                billingOptions,
                runtimeSettings));
    }
