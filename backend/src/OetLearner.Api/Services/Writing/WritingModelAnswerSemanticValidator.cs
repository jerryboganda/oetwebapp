    ...
    ILogger<WritingModelAnswerSemanticValidator> logger) : IWritingModelAnswerSemanticValidator
{
    ...
    internal static string BuildUserInput(WritingModelAnswerSemanticRequest request, string legacyLetterType) => $$"""
        ...
        8. LETTER-TYPE EVIDENCE (Addendum Two OA2-02) — the functional type comes from the case notes + the exact task, never from a catalogue code. Update-on-discharge wording is correct ONLY when the notes prove BOTH a hospital admission AND a discharge/return to ongoing care; otherwise the letter is a simple update and "presented with" is right. Never infer an admission from treatment intensity or from an LT-DG label, and never invent admission, discharge, transfer of care or a date of birth the notes do not record.
        9. READER RELEVANCE (OA2-13, OA2-18) — select what changes THIS recipient's understanding, safety, continuity or requested action; never include a fact only because it is medically interesting, and never drop a functional or safety fact to reach the word count. Allied-health recipients (occupational therapist, physiotherapist, pharmacist, radiographer) are healthcare professionals: common diagnoses such as diabetes mellitus, hypothyroidism and arthritis/arthrosis need NO lay translation — judge whether each detail matters to their functional/safety/therapy role instead.
        10. MATERIAL VITAL SIGNS (OA2-14) — when a vital sign is material to the presenting problem, the exact value and unit are stated and NOT re-labelled with a diagnosis the notes never made (report "the blood pressure was 88/70 mmHg", never "hypotension").
