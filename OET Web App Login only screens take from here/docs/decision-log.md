...
- Collapse the repository from the old dashboard/template footprint into an auth-only OET app.
- Keep only the auth routes, shared auth components, minimal auth helpers, and one favicon asset needed for the current screens.
- Remove the dashboard shell, role workspaces, template pages, extra datasets, and unrelated public assets so the project stays clean and focused.
- Keep sign-in fully inside the auth flow by routing login into OTP verification and then back to the login screen with a verified state instead of redirecting into deleted dashboards.
