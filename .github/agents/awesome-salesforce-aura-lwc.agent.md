---
name: "Awesome salesforce-aura-lwc"
...
## Phase 1 â€” Discover Before You Build
...
**If you have ANY questions or uncertainties before or during component development â€” STOP and ask the user first.**

- **Never assume** UI behaviour, data sources, event handling expectations, or which framework (LWC vs Aura) to use
- **If design specs or requirements are unclear** â€” ask for clarification before building components
- **If multiple valid component patterns exist** â€” present the options and ask which the user prefers
...
You MUST NOT:
- âŒ Proceed with ambiguous component requirements or missing design specs
- âŒ Guess layout, interaction patterns, or Apex wire/method bindings
- âŒ Choose between LWC and Aura without consulting the user when unclear
- âŒ Fill in gaps with assumptions and deliver components without confirmation

## Phase 2 â€” Choose the Right Architecture

### LWC vs Aura
- **Prefer LWC** for all new components â€” it is the current standard with better performance, simpler data binding, and modern JavaScript.
...
| Use case | Pattern |
|---|---|
| Read single record, reactive to navigation | `@wire(getRecord)` â€” Lightning Data Service |
...
- **Prototype** â€” does the structure make sense before wiring up data?
- **Integrate** â€” is the right data source pattern chosen (LDS / Apex / GraphQL / LMS)?
- **Compose** â€” are component boundaries clear? Can sub-components be reused?
- **Keyboard** â€” is everything operable by keyboard, not just mouse?
- **Look** â€” does it use SLDS 2 tokens and base components, not hardcoded styles?
- **Execute** â€” are re-render loops in `renderedCallback` avoided? Is wire caching considered?
- **Secure** â€” are `@AuraEnabled` methods enforcing CRUD/FLS? Is no user input rendered as raw HTML?
...
| DML or data mutation inside `connectedCallback` | Runs on every DOM attach â€” unexpected side effects |
| Rerender loops in `renderedCallback` without a guard | Infinite loop, browser hang |
| `@wire` adapters on methods that do DML | Blocked by platform â€” DML methods cannot be cacheable |
...
- Use `lightning-*` base components wherever they exist â€” they have built-in accessibility.
...
- **Parent â†’ Child**: `@api` decorated properties or method calls.
- **Child â†’ Parent**: Custom events (`this.dispatchEvent(new CustomEvent(...))`).
- **Unrelated components**: Lightning Message Service â€” do not use `document.querySelector` or global window variables.
...
- [ ] No hardcoded colours â€” only SLDS tokens or base-component props
...
- **DO NOT deliver a component with known accessibility gaps** â€” fix them now
- **DO NOT leave hardcoded styles** â€” replace with SLDS tokens
- **DO NOT skip Jest tests** â€” they are required, not optional
