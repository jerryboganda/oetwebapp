...
// MCQ (A/B/C), while Part C is a 4-option MCQ (A/B/C/D). Counts also differ
...
interface BuilderRow {
  ...
  stem: string;
  /** Option texts (A/B/C for Part B; A/B/C/D for Part C). */
  options: string[];
  /** Letter of the correct option: A/B/C for Part B; A/B/C/D for Part C. */
...
function buildRows(
  ...
  const letters = section.startsWith('C') ? MCQ4_LETTERS : MCQ3_LETTERS;
  const placeholders = section.startsWith('C') ? MCQ4_OPTIONS : MCQ3_OPTIONS;
  return rangeFor(section).map((number) => {
    ...
    return {
      ...
      correctAnswer: existing ? correctLetterOf(existing, letters) : '',
...
export function ListeningAnswerSheetBuilder({
  ...
  // Any item in this section authored as something other than the section's
  // governed MCQ shape was made
    ...
    try {
      ...
      const built: ListeningAuthoredQuestion[] = rows.map((row) => {
        ...
        return {
          ...
          type: activeSection.startsWith('C') ? 'multiple_choice_4' : 'multiple_choice_3',
          stem: row.stem.trim() || SEE_PDF_SENTINEL,
          options: (activeSection.startsWith('C') ? MCQ4_LETTERS : MCQ3_LETTERS)
            .map((_, i) => row.options[i]?.trim() || (activeSection.startsWith('C') ? MCQ4_OPTIONS : MCQ3_OPTIONS)[i]),
              ...
              return (
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <Badge variant="muted" size="sm" className="shrink-0">MCQ ({activeSection.startsWith('C') ? 4 : 3})</Badge>
                    <div className="flex-1 min-w-0">
                      <Select
                        aria-label={`Correct answer for question ${row.number}`}
                        placeholder="Mark correct option"
                        options={(activeSection.startsWith('C') ? MCQ4_LETTERS : MCQ3_LETTERS).map((letter, i) => ({
                          value: letter,
                  ...
                  <div className="grid gap-2">
                    {(activeSection.startsWith('C') ? MCQ4_LETTERS : MCQ3_LETTERS).map((letter, i) => (
