...
import {
  ...
  adminListSpeakingPapers,
  adminPublishPaperWithWarnings,
  adminUnarchivePaper,
  type AdminSpeakingPaperRow,
} from '@/lib/api';
...
export default function AdminSpeakingPapersPage() {
  ...
  const [rows, setRows] = useState<AdminSpeakingPaperRow[]>([]);
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const res = await adminListSpeakingPapers({
      ...
      setToast({ variant: 'error', message: err instanceof Error ? err.message : 'Failed to load speaking papers.' });
    } finally {
      setLoading(false);
    }
  ...
  const handleArchive = async (id: string) => {
    if (!canWrite) return;
    if (!confirm('Archive this speaking paper? Learners will no longer see it.')) return;
    try {
      await adminArchiveSpeakingPaper(id);
  ...
  return (
    <AdminRouteWorkspace role="main" aria-label="Speaking CMS">
      <AdminRouteHero
        eyebrow="CMS"
        icon={Mic2}
        accent="navy"
        title="Speaking CMS"
        description="Manage speaking papers — role cards, warm-up questions, assessment criteria, and AI-drafted scenarios."
        aside={canWrite ? (
          <div className="rounded-2xl border border-border bg-background-light p-4 shadow-sm">
            <div className="flex flex-wrap gap-2">
              <Button variant="outline" className="inline-flex items-center gap-2" asChild>
                <Link href="/admin/content/speaking/backfill">
                  <Wand2 className="h-4 w-4" /> Asset backfill
                </Link>
              </Button>
              <Button className="inline-flex items-center gap-2" asChild>
                <Link href="/admin/content/speaking/new">
                  <Plus className="h-4 w-4" /> New speaking paper
                </Link>
              </Button>
            </div>
          </div>
      ...
      {canPublish && draftRows.length > 0 && (
        <div className="flex flex-wrap items-center gap-2 rounded-2xl border border-border bg-background-light px-4 py-3 text-sm">
          <span className="font-medium">Bulk publish drafts:</span>
      ...
      {loading ? (
        ...
        <Card className="p-8 text-center text-sm text-muted">No speaking papers match these filters.</Card>
      ) : (
        <Card className="overflow-hidden">
          <table className="min-w-full text-sm">
            <thead className="bg-background-light text-left text-xs uppercase tracking-[0.15em] text-muted">
              <tr>
                {canPublish && <th className="px-3 py-2 w-8" aria-label="Select" />}
              ...
              {rows.map((p) => (
                <tr key={p.id} className="border-t border-border hover:bg-background-light/40">
                  ...
                  <td className="px-4 py-2">
                    <Link
                      href={`/admin/content/speaking/${p.id}`}
                  ...
                  <td className="px-4 py-2 text-right">
                    <div className="flex justify-end gap-1">
                      <Button variant="ghost" size="sm" asChild>
                        <Link href={`/admin/content/speaking/${p.id}`}>Workspace</Link>
                      </Button>
