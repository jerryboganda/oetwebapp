...
| macOS desktop auto-update **gated on signing** | `"app"` was added to `bundle.targets` (17 Sep 2026) so the build now emits the `.app.tar.gz` + `.sig` updater artifacts, and the feed assembler publishes `darwin-aarch64`/`darwin-x86_64` entries. Unsigned runs strip those entries (and `downloads.mac`) in `tauri-desktop-release.yml`, because an unsigned macOS self-update can be refused by Gatekeeper. | The macOS updater feed entry appears with the first signed+notarized release; until then Mac users install from the DMG manually (download currently disabled pending playback QA — see `APPLE-RELEASE-EVIDENCE-PACK.md`). |
