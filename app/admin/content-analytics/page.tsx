  ...
  return (
    <div className="min-h-screen bg-background-light">
      <div className="max-w-5xl mx-auto px-4 py-8">
        ...
        <p className="text-muted mb-6">Deep-dive into per-item usage, completion rates, and learner outcomes.</p>
        ...
        {loading ? <div className="grid grid-cols-4 gap-4">{Array.from({ length: 4 }).map((_, i) => <Skeleton key={i} className="h-24 rounded-xl" />)}</div> : data ? (
          <MotionSection className="space-y-6">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              <MotionItem><Card className="p-4 text-center"><BarChart3 className="w-5 h-5 mx-auto mb-2 text-primary" /><p className="text-2xl font-bold">{data.metrics.totalAttempts}</p><p className="text-xs text-muted">Total Attempts</p></Card></MotionItem>
              <MotionItem><Card className="p-4 text-center"><TrendingUp className="w-5 h-5 mx-auto mb-2 text-emerald-500" /><p className="text-2xl font-bold">{data.metrics.completionRate}%</p><p className="text-xs text-muted">Completion Rate</p></Card></MotionItem>
              <MotionItem><Card className="p-4 text-center"><Users className="w-5 h-5 mx-auto mb-2 text-primary" /><p className="text-2xl font-bold">{data.metrics.uniqueLearners}</p><p className="text-xs text-muted">Unique Learners</p></Card></MotionItem>
              <MotionItem><Card className="p-4 text-center"><Clock className="w-5 h-5 mx-auto mb-2 text-amber-500" /><p className="text-2xl font-bold">{data.metrics.averageTimeMinutes}m</p><p className="text-xs text-muted">Avg Time</p></Card></MotionItem>
            </div>

            {data.metrics.averageScore !== null && (
              <div className="grid grid-cols-3 gap-4">
                <Card className="p-4 text-center"><p className="text-xl font-bold">{data.metrics.averageScore}</p><p className="text-xs text-muted">Avg Score</p></Card>
                <Card className="p-4 text-center"><p className="text-xl font-bold">{data.metrics.medianScore}</p><p className="text-xs text-muted">Median Score</p></Card>
                <Card className="p-4 text-center"><p className="text-xl font-bold">{data.metrics.scoreStdDev}</p><p className="text-xs text-muted">Std Deviation</p></Card>
              </div>
            )}
