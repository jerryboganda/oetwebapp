...
- If architectural_decisions found: delegate to `gem-documentation-writer` â†’ create/update `PRD`
...
- new_task â†’ Phase 2
- continue_plan + feedback â†’ Phase 2 (adjust plan based on feedback)
...
- If validation fails:
  - Failed + replanable â†’ delegate to `gem-planner` with findings for replan.
...
- Pre-Wave:
  - Check memory for known `failure_modes` and `gotchas` of similar tasks â†’ add guards to task definition.
- Execute Waves:
...
- Integration Check:
  ...
  - ui|ux|design|interface|a11y tasks â†’ validate with the designer agent matching the task's assigned agent (if task.agent is `designer-mobile`, use `gem-designer-mobile(validate)`; otherwise use `gem-designer(validate)`), run in parallel with `gem-reviewer(wave scope)`.
  - If reviewer fails â†’ `gem-debugger` to diagnose:
    - If debugger confidence â‰¥ 0.85 â†’ delegate to `gem-implementer` with diagnosis â†’ re-verify.
    - If debugger confidence < 0.85 â†’ escalate to user (cannot reliably diagnose).
  - If designer validation fails â†’ mark task as `needs_revision`, append design findings to task definition, and flag for re-design.
  - Synthesize statuses (completed / escalate / needs_replan). Persist all to `plan.yaml`.
- Loop:
  - After each wave â†’ Phase 4 â†’ immediately next.
  - Blocked â†’ Escalate.
...
- Collect & Merge:
  ...
  - Promote: `gotchas` recurring â‰¥ 3Ã— across plans â†’ `patterns`. `failure_modes` recurring â‰¥ 2Ã— â†’ elevate severity.
- Memory:
...
- Conventions:
  - If `conventions` found: delegate to `gem-documentation-writer` â†’ create/update `AGENTS.md`
- Decisions:
  - If `decisions` found: delegate to `gem-documentation-writer` â†’ create/update `PRD`
- Skills:
  - If `patterns` with confidence â‰¥ 0.85 AND non-trivial: delegate to `gem-skill-creator`.
...
```jsonc
{
  ...
  "security_sensitive_tasks": ["string â€” task IDs requiring per-task deep scan (merged into wave review)"],
