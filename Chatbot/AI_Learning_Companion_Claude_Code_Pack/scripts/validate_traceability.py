...
errors=[]
with (ROOT/'traceability/features.json').open(encoding='utf-8') as f:
    data=json.load(f)
...
with (ROOT/'traceability/features.csv').open(encoding='utf-8-sig', newline='') as f:
    rows=list(csv.DictReader(f))
csv_ids=[r.get('id') for r in rows]
if csv_ids != expected:
    errors.append(f'CSV inventory/order mismatch; count={len(csv_ids)}')
md=(ROOT/'traceability/FEATURE_TRACEABILITY_MATRIX.md').read_text(encoding='utf-8')
