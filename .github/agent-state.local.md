...
## Current task — Entitlement integrity: admin credits, master access cap, one-time renewal labels — SHIPPED (this commit)
- Spec implemented: admin AI-credit Add/Remove/Set-Exact semantics verified and regression-tested (Total-only changes, Used = genuine consumption, set-exact below used rejected); course-removal revocation now provably revokes only the UNUSED course-granted credits while preserving recorded usage and separately admin-granted credits; Master Access Expiry is a real sticky global cap (`LearnerUser.AccessExpiresAtIsAdminCap`, min-semantics in SyncAccessExpiryAsync — package mutations tighten but never extend; clearing removes the gate until next mutation re-mirrors); one-time packages no longer fabricate `NextRenewalAt` (mirrors `ExpiresAt`) and lifecycle "renews soon" notifications are renewable-plan-only; UI labels switched from "Next renewal" to "Access expires/ends" with endDate preference (SubscriptionCard, account/billing, admin user page, admin package cards capped by master expiry with amber warning; manage-access-panel passes `masterAccessExpiresAt` and documents cap semantics).
- Migration `20260829090000_AdminMasterAccessCapAndOneTimeRenewal` adds `AccessExpiresAtIsAdminCap` (bool, default false) to "Users"; registered in MigrationHygieneTests.
- Validation (clean main worktree): `dotnet build` 0 errors; focused backend suites 82/82 (AiPackageCreditService + UserAccessAllocationService + MigrationHygiene); `pnpm run ship:gate` OK. Repo-wide `tsc --noEmit` has pre-existing unrelated errors (admin/policies/reading, expert listening review, listening results, reading paper/practice, settings/ai, credit-bucket-adjuster, whop); dashboard `app/page.test.tsx` failures are pre-existing (reproduced 22 failures with the change stashed); mobile SubscriptionManager intentionally unchanged (separate data shape).
- Admin endpoints verified already-correct (no change): AiPackageCreditService.AdjustAsync (Total-only admin deltas, `credits_total_below_used` guard), ReverseGrants/ReverseOneGrant (unused-only per source), UpdatePackageDatesAsync (absolute replacement, past-end expires + syncs lots).

## Previous task — Listening Part B/C release-blocking fixes — DEPLOYED + LIVE / QA BOUNDARIES OPEN
