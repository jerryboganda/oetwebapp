...
public class NativeIapEndpointsTests : IClassFixture<TestWebApplicationFactory>
{
    ...
    public async Task AdminMappings_ValidatePlatformAndRejectDuplicateActiveProduct()
    {
        ...
        var productId = $"com.oetprep.test.plan.{Guid.NewGuid():N}";
    ...
    public async Task LearnerProducts_ReturnOnlyActiveMappingsForRequestedPlatform()
    {
        ...
        db.NativeIapProductMappings.AddRange(
            NewMapping($"ios-active-{suffix}", "ios", $"com.oetprep.ios.active.{suffix}", true, now),
            NewMapping($"ios-inactive-{suffix}", "ios", $"com.oetprep.ios.inactive.{suffix}", false, now),
            NewMapping($"android-active-{suffix}", "android", $"com.oetprep.android.active.{suffix}", true, now));
        ...
        Assert.Contains(items, item => item.GetProperty("storeProductId").GetString() == $"com.oetprep.ios.active.{suffix}");
        Assert.DoesNotContain(items, item => item.GetProperty("storeProductId").GetString() == $"com.oetprep.ios.inactive.{suffix}");
    ...
    public async Task AdminMappings_WriteAuditEventsForCreateUpdateAndDelete()
    {
        using var admin = CreateAdminClient();
        await EnsureActivePlanAsync("starter");
        var productId = $"com.oetprep.audit.{Guid.NewGuid():N}";
    ...
    public async Task ReceiptValidation_FailsClosedWithoutGrantingEntitlementOrEchoingRawToken()
    {
        ...
        {
            platform = "ios",
            storeProductId = "com.oetprep.test.plan",
