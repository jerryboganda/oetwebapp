...
# React 18 Commander - Migration Orchestrator (React 16/17 â†’ 18.3.1)
...
- componentWillMount â†’ componentDidMount (or state â†’ constructor)
- componentWillReceiveProps â†’ getDerivedStateFromProps or componentDidUpdate
- componentWillUpdate â†’ getSnapshotBeforeUpdate or componentDidUpdate
- Legacy Context (contextTypes/childContextTypes/getChildContext) â†’ createContext
- String refs (this.refs.x) â†’ React.createRef()
- findDOMNode â†’ direct refs
- ReactDOM.render â†’ createRoot (needed to enable auto-batching + React 18 features)
...
**Gate:** npm test â†’ 0 failures, 0 errors.
...
## Why This Is Harder Than 18 â†’ 19
...
- **Automatic batching** is the #1 silent runtime breaker. `setState` in Promises or `setTimeout` used to trigger immediate re-renders. Now they batch. Class components with async data-fetch â†’ setState â†’ conditional setState chains WILL break.
...
- **Event delegation** changed in React 17: events moved from `document` to the root container. If the team went 16 â†’ minor patches â†’ 18 without a proper 17 migration, there may be `document.addEventListener` patterns that now miss events.
...
- [ ] componentWillMount â†’ componentDidMount / constructor
- [ ] componentWillReceiveProps â†’ getDerivedStateFromProps / componentDidUpdate
- [ ] componentWillUpdate â†’ getSnapshotBeforeUpdate / componentDidUpdate
- [ ] Legacy context â†’ createContext
- [ ] String refs â†’ React.createRef()
- [ ] findDOMNode â†’ direct refs
