---
name: "Awesome cast-imaging-software-discovery"
...
**Tool sequence**: `applications` â†’ `stats` â†’ `architectural_graph` |
  â†’ `quality_insights`
  â†’ `transactions`
  â†’ `data_graphs`
...
**Tool sequence**: `stats` â†’ `architectural_graph` â†’ `objects` â†’ `object_details`
...
**Tool sequence**: |
  â†’ `packages` â†’ `package_interactions`  â†’ `object_details`
  â†’ `inter_applications_dependencies`
...
**Tool sequence**: `application_database_explorer` â†’ `object_details` (on tables)
...
**Tool sequence**: `source_files` â†’ `source_file_details`
