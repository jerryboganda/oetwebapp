...
describe("oet routing helpers", () => {
  it("maps each role to its primary landing route", () => {
    ...
    expect(resolveRoleLandingPath("expert")).toBe("/reviewer/queue");
    expect(resolveRoleLandingPath("admin")).toBe("/cms/content");
  });

  it("infers the protected role surface from the pathname", () => {
    expect(getRoleFromPath("/learner/writing/tasks")).toBe("learner");
    expect(getRoleFromPath("/reviewer/review/writing/rev_1")).toBe("expert");
    expect(getRoleFromPath("/expert/review/writing/rev_1")).toBe("expert");
    expect(getRoleFromPath("/cms/analytics/quality")).toBe("admin");
  ...
  it("only grants access to the matching protected surface", () => {
    ...
    expect(canAccessRolePath("learner", "/reviewer/queue")).toBe(false);
    expect(canAccessRolePath("expert", "/reviewer/queue")).toBe(true);
    expect(canAccessRolePath("expert", "/cms/content")).toBe(false);
    expect(canAccessRolePath("admin", "/cms/content")).toBe(true);
