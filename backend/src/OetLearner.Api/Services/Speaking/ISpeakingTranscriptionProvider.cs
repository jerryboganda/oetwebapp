...
///         schema documented on <see cref="OetLearner.Api.Domain.SpeakingTranscript.SegmentsJson"/>:
...
public interface ISpeakingTranscriptionProvider
{
    ...
    /// <see cref="OetLearner.Api.Domain.SpeakingTranscript.Provider"/>
...
/// <see cref="OetLearner.Api.Domain.SpeakingTranscript"/> so the pipeline
...
public sealed record SpeakingTranscriptionProviderResult
{
    ...
    /// <see cref="OetLearner.Api.Domain.SpeakingTranscript.Provider"/>.</summary>
    public required string Provider { get; init; }

    /// <summary>ISO-639-1 code the provider actually used. Persisted on
    /// <see cref="OetLearner.Api.Domain.SpeakingTranscript.Language"/>.</summary>
    public required string Language { get; init; }

    /// <summary>JSON array of segment objects with shape
    /// <c>{ speaker, startMs, endMs, text, confidence, words[] }</c>.
    /// Persisted verbatim into <see cref="OetLearner.Api.Domain.SpeakingTranscript.SegmentsJson"/>.</summary>
    public required string SegmentsJson { get; init; }

    /// <summary>Total word count over the transcript. Persisted on
    /// <see cref="OetLearner.Api.Domain.SpeakingTranscript.WordCount"/>.</summary>
    public required int WordCount { get; init; }

    /// <summary>Mean confidence over the transcript segments, in [0, 1].
    /// Persisted on
    /// <see cref="OetLearner.Api.Domain.SpeakingTranscript.MeanConfidence"/>.</summary>
...
public sealed record SpeakingTranscriptionStatus
{
    ...
    /// <summary>FK to the latest <see cref="OetLearner.Api.Domain.SpeakingTranscript"/>
