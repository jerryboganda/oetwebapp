---
name: "Awesome stackhawk-security-onboarding"
...
**Decision Logic:**
- If already configured â†’ offer to review/update
- If clearly a library/docs â†’ politely decline and explain why
- If application with sensitive data â†’ proceed with high priority
- If application without sensitive data findings â†’ proceed with standard setup
- If uncertain â†’ ask the user if this repo serves an API or web application
...
**Configuration Logic:**
- If host clearly detected â†’ use it
- If host ambiguous â†’ default to `http://localhost:3000` with TODO comment
- If auth mechanism detected â†’ configure appropriate type with TODO for credentials
- If auth unclear â†’ omit auth section, add TODO in PR description
...
ðŸŽ¯ **Risk Assessment:** This repository was identified as a candidate for security testing based on:
