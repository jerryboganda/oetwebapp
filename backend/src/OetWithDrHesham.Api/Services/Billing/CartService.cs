...
public sealed class CartService : ICartService
{
    ...
    public async Task<CartDto> ApplyPromoCodeAsync(string cartId, string userId, string code, CancellationToken ct = default)
    {
        ...
        var coupon = await _db.BillingCoupons
            .Where(c => c.Code == code && c.Status == OetWithDrHesham.Api.Domain.BillingCouponStatus.Active)
        ...
        decimal? discountAmount = coupon.DiscountType == OetWithDrHesham.Api.Domain.BillingDiscountType.FixedAmount
            ? coupon.DiscountValue : null;
        decimal? discountPercent = coupon.DiscountType == OetWithDrHesham.Api.Domain.BillingDiscountType.Percentage
            ? coupon.DiscountValue : null;
