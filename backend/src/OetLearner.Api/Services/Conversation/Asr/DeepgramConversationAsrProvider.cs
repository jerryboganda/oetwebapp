...
/// <summary>
/// Deepgram pre-recorded transcription for conversation audio. Deepgram's
/// streaming WebSocket can be added later; this REST client handles per-turn
/// audio blobs under ~60 s reliably.
/// </summary>
    ...
    ILogger<DeepgramConversationAsrProvider> logger) : IConversationAsrProvider
{
    ...
    {
        if (!IsConfigured)
            throw new InvalidOperationException("Deepgram provider is not configured.");
        ...
        {
            logger.LogWarning("Deepgram ASR returned {Status}: {Body}", (int)response.StatusCode, body);
        ...
        {
            var top = channels[0];
            if (top.TryGetProperty("alternatives", out var alts) &&
        ...
        return new ConversationAsrResult(
            ...
            Language: lang,
            ProviderName: Name,
            ProviderResponseSummary: $"deepgram: {text.Length} chars");
    }
}
