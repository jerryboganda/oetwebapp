...
CocoaPods-sourced transitive pods were previously **UNKNOWN from the repository** because `Podfile.lock` was not committed (gap #11). The lock is now committed — generated by the first successful `iOS Build Check` run — and it pins them:

| Transitive pod | Resolved | Required by |
| --- | --- | --- |
| `IONFilesystemLib` | 1.1.4 | `CapacitorFilesystem` (`~> 1.1.1`) |
| `KeychainSwift` | 21.0.0 | `AparajitaCapacitorSecureStorage` (`~> 21.0`) |

`GCDWebServer` does **not** appear: it is only pulled in by `capacitor-blob-writer`, a transitive npm dependency of `capacitor-voice-recorder` that this project's `Podfile` never lists — so it was never part of the iOS build, and the earlier concern about its deployment target was unfounded. All 21 pods now carry `SPEC CHECKSUMS`, so the resolved graph is reproducible rather than re-resolved on every run.
