    ...
    <>
      <section className="rounded-2xl border border-[#D4A44F]/40 bg-[#D4A44F]/5 p-5 shadow-sm dark:border-[#D4A44F]/30">
        <header className="flex items-center gap-2">
          <Sparkles className="h-4 w-4 text-[#996F1F]" aria-hidden="true" />
          ...
          {visibleAddOns.map((addon) => (
            ...
            >
              <div className="flex items-start justify-between gap-2">
                <div>
                  <h4 className="text-sm font-bold text-navy">{addon.name}</h4>
                ...
                <div className="text-right">
                  <div className="font-bold text-navy">£{addon.price.toFixed(0)}</div>
                  {addon.originalPrice !== null && addon.originalPrice !== undefined && addon.originalPrice > addon.price && (
                    <div className="text-[10px] text-muted line-through">£{addon.originalPrice.toFixed(0)}</div>
                  )}
                </div>
              </div>
              {addon.description && (
                <p className="mt-2 text-xs text-muted line-clamp-2">{addon.description}</p>
              )}
              <button
                type="button"
                onClick={() => {
              ...
              >
                <ShoppingBag className="h-3 w-3" aria-hidden="true" /> Add to my course
              </button>
            </article>
          ))}
        </div>
