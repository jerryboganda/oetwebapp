...
- `ReadingLearnerEndpoints.cs` — `/v1/reading-papers`: learner Reading practice. Projection layer must never serialize `CorrectAnswerJson` / `ExplanationMarkdown` / `AcceptedSynonymsJson` to learner-facing payloads. PM-001 was reconciled on 2026-05-29 by redacting submitted-review answer/explanation fields from learner payloads.
...
| --- | --- | --- | --- |
| PM-001 | Reading review | Reconciled locally on 2026-05-29: submitted learner review payloads no longer emit `CorrectAnswer` or `ExplanationMarkdown`; answer-key-only data remains server-side/admin-only. | Keep focused regression coverage before Reading release sign-off. |
...
| PM-016 | Reading PM-001 reconciliation | Reconciled locally on 2026-05-29; keep the AGENTS hard ban (no `CorrectAnswerJson` / `ExplanationMarkdown` / `AcceptedSynonymsJson` to learners) as the invariant for future Reading review changes. | Reading release owner |
