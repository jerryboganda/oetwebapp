...
| BUG-007 | High | **OPEN/DEFERRED** | Bundled .NET backend pulls `SQLitePCLRaw.lib.e_sqlite3` 2.1.11 with a known **high-severity** advisory (NU1903 / GHSA-2m69-gcr7-jv3q) — ships inside the desktop installer | Phase 9 build | Surfaced by `dotnet publish` during the desktop build. The native SQLite lib ships in the desktop bundle (offline SQLite). **Fix is a backend dependency bump** (`SQLitePCLRaw.*` → patched e_sqlite3) — **out of this `src-tauri`-only branch's scope.** Recommend the backend team bump it before shipping the desktop build to testers. |
