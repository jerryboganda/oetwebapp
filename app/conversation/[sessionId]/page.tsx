...
export default function ConversationSessionPage() {
  ...
  // Cleanup: stop hub + stop audio on unmount
  useEffect(() => {
    return () => {
      if (prepTimerRef.current) clearInterval(prepTimerRef.current);
      if (timerRef.current) clearInterval(timerRef.current);
  ...
  const handleStartConversation = useCallback(async () => {
    ...
    try {
      ...
      connection.on('ReceiveTranscript', (turnNumber: number, text: string, confidence?: number, meta?: ConversationTranscriptMeta) => {
        void confidence;
        setTurns((prev) => [
          ...prev,
          {
      ...
      connection.on('ReceiveAIResponse', (turnNumber: number, text: string, meta?: ConversationAiMeta) => {
        ...
        setTurns((prev) => [
          ...prev,
          {
            ...
            timestamp: Date.now(),
            audioUrl: meta?.audioUrl ?? null,
            appliedRuleIds: meta?.appliedRuleIds ?? [],
          },
        ]);
      ...
      connection.onclose(() => {
        // surface disconnection but don't crash the UI
        console.debug('ConversationHub: closed');
      });
  ...
  const handleRecord = useCallback(async () => {
    ...
    try {
      ...
      mediaRecorder.onstop = async () => {
        ...
        reader.onloadend = async () => {
          ...
          if (hubRef.current && base64) {
            try {
              await hubRef.current.invoke('SendAudio', sessionId, base64, 'audio/webm');
            } catch {
      ...
      // Auto-stop at MAX_TURN_MS so the server never receives more than it allows.
      setTimeout(() => {
        ...
        {state === 'evaluating' && (
          <MotionSection className="text-center py-16">
            ...
            <p className="text-gray-500">
              Our AI is analysing your performance. This usually takes a few seconds.
            </p>
          </MotionSection>
        )}
