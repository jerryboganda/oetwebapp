...
## Current task — Writing Submit-for-Grading P0 systemic repair (master acceptance prompt) — IMPLEMENTED + TARGETED TESTS GREEN, NOT COMMITTED (master §30: no push/deploy without explicit authorization)
- **Root cause (proven)**: `profession_pack_not_approved` reaches learners because the publish gate (`WritingTaskAuthoringService.ValidateAsync`) only checked filesystem rulebooks while runtime grading (`WritingAssessmentPreflightService`) required an exact approved+candidate-facing `WritingAssessmentPackVersions` row; `WritingScenarioService.ApproveAsync` + `WritingTaskProjectionService` published with zero validation. No shared resolver existed, so validator said valid while runtime said invalid.
- **Fix (13 backend + 3 frontend files, 3 new)**: shared `ResolvePackAsync` fallback (exact → profession generic `other`/`routine_referral` → any-for-profession; `transfer`/`referral_to_gp` keep genuine detailed-pack requirement; never cross-profession); recipient/diagnosis/conflict softened at runtime (warn+proceed) and hardened at publication (new blocking codes `profession_pack_unapproved`, `recipient_unresolved`, `diagnosis_or_request_unresolved`, `task_classification_conflict`); Submit pipeline no longer generates Model Answers (dep removed) and no longer leaks internal codes in messages; `WritingSubmissionService` dedupes (key+content window) before the terminal-state lock; projection normalizes taxonomy + preserves authored fields + holds task-less rows as draft; new `WritingCataloguePreflightService` + `GET catalogue-compatibility` + `POST catalogue-quarantine` (dry-run default) endpoints, DI-registered.
- **Tests**: 59/59 green across 9 Writing submit/grading suites (incl. 3 new files: preflight fallback/Other-Letters/never-default tests, 78-row full-matrix catalogue tests, submit-path single-call/idempotency/safe-error tests); 49/49 neighboring suites green; full `~Writing` 609/611 with only 2 pre-existing rule-engine failures (proven on clean HEAD via stash). Frontend `submit-keys` 8/8 green via isolated vitest config (workspace jsdom/eslint installs broken — pre-existing env issue); `tsc --noEmit` clean on all touched frontend files.
- **Artifacts**: `artifacts/writing-submit-p0-acceptance/acceptance-report.json` (78/78 PASS synthetic full matrix) + `.csv`. Production catalogue scan still required via the new endpoint (expected invalids per the known 194-task case-notes gap) + backfill/quarantine before release → **RELEASE BLOCKED pending prod scan** (honest gate status, see final report).
- **Next**: run `GET /v1/admin/writing/tasks/catalogue-compatibility` against prod, backfill case notes + approve packs/exemplars, re-run to 0 invalid, then ship per Ship-It workflow ONLY when explicitly authorized. Do NOT stage unrelated dirty files.
- **SHIP STATUS 2026-09-05 (SHA f3cd6e21c)**: ✅ DEPLOYED + LIVE (owner authorized push+deploy). `ship:gate` OK → committed 22 scoped files (unrelated dirty files preserved unstaged) → repo public → pushed `fa2ad95ac..f3cd6e21c` → `Build & Deploy` SUCCESS → VPS `web-green`+`api-green` on `f3cd6e21c`, all healthy → repo PRIVATE → live `api/health/ready` 200, `app/api/health` 200 → new prod routes verified serving+auth-gated (401 anon, not 404).
- **LIVE CATALOGUE SCAN 2026-09-05 (owner-supplied admin creds, used once via env, cleared after; tokens never logged)**: `GET /v1/admin/writing/tasks/catalogue-compatibility` → **194 published / 0 ready / 194 invalid / releaseBlocked=true**. Blockers: `model_answer_not_approved`=182, `profession_pack_not_approved`=179, `diagnosis_or_request_unresolved`=31, `letter_type_pack_not_approved`=15 (all LT-TR), `recipient_unresolved`=7, `task_classification_conflict`=7. Canonical case notes + task prompts present on all (no `case_note_pages*` codes). Artifact: `artifacts/writing-submit-p0-acceptance/live-catalogue-2026-09-05.json` (committed locally as e968bb32e on `writing/final-production-release` — NOT pushed: main has since advanced with parallel sessions' PRs #189–#191 + `4680fb13f`, and this worktree now sits on their branch with their dirty files; pushing would collide).
- **PROD SUPERSEDED (healthy)**: VPS now runs `53eb722d`/`007639c8` (parallel sessions redeployed; all slots healthy). Re-ran the catalogue scan against current prod code → **row-identical outcome (diff=0)**. No paid submit attempted: zero publish-ready scenarios exist, so any submit would correctly fail-closed; empty-submit proof deferred until ≥1 task is repaired.
- **Next (needs owner authorization, do NOT self-execute)**: (1) approve packs (`other`+`transfer` per profession × 6 = 12, with owner approval-evidence text), (2) generate+approve 182 exemplars (real Sonnet spend — needs consent), (3) human-fix 31+7+7 content rows, (4) rescan to 0 invalid, (5) reconcile/merge with `main` (#189–#191 reworked the same submit path) before the next deploy. Quarantine NOT run (would empty the live catalogue).
- **Touched (stage ONLY these when authorized)**: `backend/src/.../Writing{AssessmentPreflightService,ScenarioService,SubmissionEvaluationPipeline,SubmissionService,TaskAuthoringService,TaskProjectionService,CataloguePreflightService(new)}`, `Endpoints/WritingTaskAdminEndpoints.cs`, `Program.cs`, `backend/tests/.../Writing{AssessmentPreflightTests,ExamClosureTests,CatalogueCompatibilityTests(new),SubmitGradingPathTests(new)}`, `lib/writing/submit-keys{,.test}.ts`, `app/writing/practice/session/[scenarioId]/page.tsx`, `app/writing/submissions/[id]/grading/page.tsx`, `artifacts/writing-submit-p0-acceptance/*`.

## Previous task — App release playbook ("cut app releases" = Android + iOS + Desktop) — WRITTEN, NOT COMMITTED
...
- **Fix** (`AiPackageCreditService` central + wiring, no migration, existing indexes cover new queries):
  ...
  - New `Park/UnparkSubscriptionLotsAsync` (reversible, balances preserved; reversed/consumed lots never revived) wired into Suspend/Restore/UpdatePackageDates/billing Cancel-immediate/SetStatus/Reactivate.
  - `ReverseOneGrantAsync`: skips Expired lots; parked lots don't poison the idempotency marker.
  - Read gates fail closed for synthetic-legacy unlimited; `ExpireIfNeeded` + `AdjustAsync` repair stuck nulls.
  - `GrantAddonAsync`: rejects non-window parents (pre-start/post-expiry ghost).
...
## Previous task — Writing Assessment: Fix false missing recipient block on submit (Doris White) — VERIFIED + READY TO SHIP
...
## Current task — "App not installed" on Android update — ROOT-CAUSED + 1.4.10 CUT
- Root cause PROVEN by APK forensics + Play API: Play delivery cert D2:8D:…:46:B3 vs VPS/upload cert 41:5F:…:7F:A9 — different keys. VPS lineage internally consistent (same signer 1.4.7 vc2 → 1.4.8 vc3 → 1.4.9 vc4, same package, identical manifest). Cross-channel installs can never update (OS signature enforcement); no app code can override it.
- Fixes committed (9cc345ac0 + 54b2bee02 annotation fix, both on origin/main): InstallerSource native plugin + channel-aware android-install page (Play copies go to Play listing, APK hidden from them); build.gradle fails release builds closed without keystore; mobile-release.yml aborts unless APK carries pinned upload cert; play-store-automation.md documents single-channel policy.
