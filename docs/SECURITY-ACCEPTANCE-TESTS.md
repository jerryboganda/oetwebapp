...
|---|---|---|---|---|---|---|
| T1 | A standard screenshot attempt produces a black/protected result, or the video is hidden immediately. | Expected: Pass (`WDA_EXCLUDEFROMCAPTURE`) | **CI-proven Pass** on macOS 15.7.9 and 26.6.2 (GitHub-hosted runner, real Apple hardware, run [35136291136](https://github.com/jerryboganda/oetwebapp/actions/runs/35136291136), 16 Sep 2026: `screencapture -x` and ScreenCaptureKit stills both hidden). **Physical, phone-filmed Mac proof still required** — owner acceptance bar, 17 Sep 2026. | Expected: Pass (`FLAG_SECURE`) | **Expected: Fail to block, Pass to react** — iOS reports the still capture, then the app pauses/mutes and the server revokes the playback session. A signed iOS build is still unavailable for hardware proof. | **Platform limitation** — browsers expose no screenshot-block API; verify the identifying watermark is present in the captured frame. |
| T2 | Starting a built-in or third-party screen recorder pauses/blanks the video and stops audio. | Expected: Pass | **FAILED on macOS 15.7.9** (real CI run above: `screencapture -V` recording captured the protected window). **Pass on macOS 26.6.2** in the same run — OS-version-dependent, not a flake. Matches Apple's own guidance that `NSWindow.sharingType=.none` is documented as a legacy mechanism ScreenCaptureKit recorders may ignore. **Release of any macOS desktop build is held by the owner (17 Sep 2026) until this is Pass, on a phone-filmed physical Mac, on every officially supported macOS version.** See `SECURITY-CONTENT-PROTECTION-MATRIX.md` §3a for the FairPlay/DRM plan now required to close this. | Expected: Pass (recording is also blocked by FLAG_SECURE, produces black) | Expected: Pass (`enableCaptureBlackout` on `UIScreen.isCaptured`) | **Platform limitation** — no browser capture-state API; watermark/session/audit controls compensate. |
| T3 | OBS, screen mirroring, remote desktop, and common capture tools tested on each desktop platform; limitations reported with compensating control. | OBS: rides the same capture API, expected blocked. Remote desktop: **not separately tested** — some RDP shadowing modes use a different scan-out path; flag as unverified until tested. | **Not tested at all** (OBS, AirPlay/QuickTime mirroring, Zoom, Google Meet). Given T2's macOS 15 failure, do not assume Pass for any of these — Apple documents ScreenCaptureKit clients (which includes OBS, Zoom, Meet, Teams) as behaving inconsistently with `sharingType=.none` even relative to each other. Also found in the same CI run: the Bunny player's Picture-in-Picture control is still present and unverified whether it escapes protection (owner gate item). | — | — | Browser-level capture is not blockable; watermark evidence is required. |
