---
name: "Awesome react18-test-guardian"
description: 'Test suite fixer and verifier for React 16/17 â†’ 18.3.1 migration. Handles RTL v14 async act() changes, automatic batching test regressions, StrictMode double-invoke count updates, and Enzyme â†’ RTL rewrites if Enzyme is present. Loops until zero test failures. Invoked as subagent by react18-commander.'
...
```bash
npm test -- --watchAll=false --passWithNoTests --forceExit 2>&1 | grep "FAIL\|â—" | head -30
```

Group failures by category:

- Enzyme failures â†’ T-Enzyme block
- `act()` warnings/failures â†’ T1
- State assertion timing â†’ T2
- `userEvent not awaited` â†’ T3
- Call count assertion â†’ T4
- Apollo mock timing â†’ T6
...
| `Cannot read getByText of undefined` | Enzyme wrapper â‰  screen | Switch to RTL queries |
