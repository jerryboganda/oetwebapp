...
import {
  askListeningQuestionGroundedAi,
  type ListeningQuestionQnaMessage,
  type ListeningQuestionQnaResponse,
...
export function GroundedListeningQuestionQna({
  ...
  const [history, setHistory] = useState<ListeningQuestionQnaMessage[]>([]);
  const [result, setResult] = useState<ListeningQuestionQnaResponse | null>(null);
  ...
  const ask = async () => {
    ...
    setError(null);
    try {
      const response = await askListeningQuestionGroundedAi(attemptId, questionId, trimmed, history);
      setResult(response);
      setHistory(response.history);
      setMessage('');
    } catch (err) {
      setError(readErrorMessage(err, 'Grounded Listening Q&A is not available for this question yet.'));
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="mt-4 rounded-xl border border-sky-200 bg-sky-50/70 p-4 dark:border-sky-400/30 dark:bg-sky-950/20" data-testid="listening-grounded-qna">
      <div>
        <p className="text-[11px] font-black uppercase tracking-[0.14em] text-sky-700 dark:text-sky-300">Ask about this Listening evidence</p>
      ...
      <div className="mt-3 flex flex-col gap-2 sm:flex-row sm:items-end">
        <label className="flex-1">
          <span className="sr-only">Ask a question about this Listening evidence</span>
          <textarea
            value={message}
            ...
            maxLength={2000}
            rows={2}
            placeholder="Why does the transcript support this answer?"
      ...
      {result ? (
        <div className="mt-3 rounded-lg border border-sky-200 bg-white/70 p-3 text-sm leading-6 text-navy dark:border-sky-400/30 dark:bg-sky-950/30 dark:text-white/90" role="status" aria-live="polite">
          <p>{result.reply}</p>
          <p className="mt-2 text-[11px] text-muted">Answer grounded in approved Listening evidence; advisory only and marks are unaffected.</p>
        </div>
      ) : null}
    </div>
  );
