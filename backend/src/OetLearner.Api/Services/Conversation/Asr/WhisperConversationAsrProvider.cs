...
/// <summary>
/// OpenAI-compatible Whisper transcription for conversation audio. Returns
/// transcript + language only — we do not need phoneme-level alignment here
/// (that lives in the Pronunciation subsystem).
/// </summary>
    ...
    ILogger<WhisperConversationAsrProvider> logger) : IConversationAsrProvider
{
    ...
    public async Task<ConversationAsrResult> TranscribeAsync(ConversationAsrRequest request, CancellationToken ct)
    {
        ...
        {
            logger.LogWarning("Conversation Whisper ASR returned {Status}: {Body}", (int)response.StatusCode, body);
        ...
        // Whisper verbose JSON doesn't provide a holistic confidence; approximate from
        // the average of per-segment avg_logprob if present.
        ...
        return new ConversationAsrResult(
            ...
            DurationMs: duration.HasValue ? (int)(duration.Value * 1000) : 0,
            Language: lang,
            ProviderName: Name,
            ProviderResponseSummary: $"whisper: {text.Length} chars, dur={duration}");
    }
