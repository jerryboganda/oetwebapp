...
{
    Task SendAsync(OetWithDrHesham.Api.Domain.PushSubscription subscription, string payload, CancellationToken cancellationToken = default);
}
...
public sealed class WebPushDispatcher(IOptions<WebPushOptions> options, IRuntimeSettingsProvider runtimeSettingsProvider) : IWebPushDispatcher
{
    ...
    public async Task SendAsync(OetWithDrHesham.Api.Domain.PushSubscription subscription, string payload, CancellationToken cancellationToken = default)
    {
        try
        {
