...
| P2-4 | Reading rulebook B section 7 ("rules and regulations your academy must reflect") is partially implemented. "No use/disclosure of real test content" academic-integrity policy exists in terms but isn't surfaced inside the practice-test mode (no inline reminder). | `app/terms/page.tsx` covers it generically; no inline mock-mode policy banner. | **Implemented in this branch.** Mock-mode banner (`ReadingIntegrityBanner`) renders on the Part A launch screen, above the timer during active Part A, and on the break screen. Pinned by [tests/e2e/reading/integrity-banner.spec.ts](../../oetwebapp/tests/e2e/reading/integrity-banner.spec.ts). |
| P2-5 | Reading rulebook B section 6 ("Estimated OET band ... 'This is an estimate, not an official OET conversion.'") — present in code (`lib/scoring.ts#L780-L795`) but the literal disclaimer string was not verified rendered on `/reading/paper/.../results`. | `app/reading/paper/[paperId]/results/page.tsx#L99-L260`. | **Verified literal disclaimer present.** This branch appends the verbatim string "This is an estimate, not an official OET conversion." directly under the score-card row, tagged `data-testid="reading-scoring-disclaimer"`. |
...
| R10.8 **CRITICAL** | B/C: do NOT highlight choices | ✅ | Highlighter is scoped via data attributes; choices carry `data-reading-answer-choice="true"` and never `data-reading-highlight-scope`. Pinned by [tests/e2e/reading/highlight-restriction.spec.ts](../../oetwebapp/tests/e2e/reading/highlight-restriction.spec.ts) (audit branch `audit/rulebook-compliance-2026-05-10`). |
...
| 7 | Rules platform must teach (10 rules) | ✅ | 10/10 enforced after audit branch lands the mock-mode integrity banner (Part A launch + active Part A toolbar + break screen) and pins it via [tests/e2e/reading/integrity-banner.spec.ts](../../oetwebapp/tests/e2e/reading/integrity-banner.spec.ts). |
