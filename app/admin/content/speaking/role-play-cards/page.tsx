 ...
 *   - AdminRouteWorkspace + AdminRouteHero with a "New role-play card"
 *     toolbar button
...
import { ClipboardList, Copy, Plus, RotateCcw } from 'lucide-react';
import {
  AdminRouteHero,
  AdminRoutePanel,
  AdminRouteWorkspace,
...
export default function AdminSpeakingRolePlayCardsPage() {
  ...
  return (
    <AdminRouteWorkspace role="main" aria-label="Speaking role-play cards">
      <AdminRouteHero
        eyebrow="CMS"
        icon={ClipboardList}
        accent="navy"
      ...
      {loading ? (
        ...
        <Card className="p-8 text-center text-muted">
          No role-play cards yet. Click <strong>New role-play card</strong> to author the
          first one.
        </Card>
      ) : (
        <div className="space-y-3">
          {rows.map((row) => (
            <Card key={row.cardId} className="p-4">
                ...
                <div className="min-w-0 flex-1">
                  <div className="mb-1 flex flex-wrap items-center gap-2">
                    ...
                    {row.hasInterlocutorScript ? (
                      <Badge variant="success">interlocutor ready</Badge>
                    ) : (
                      <Badge variant="muted">no interlocutor</Badge>
                    )}
                    {row.isLiveTutorEligible ? (
                      <Badge variant="outline">live-tutor</Badge>
                    ) : null}
                  </div>
