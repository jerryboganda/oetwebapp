...
# ssh uses -p for port; scp uses -P. Sharing one array made scp treat "22" as a file.
common_opts=(
  -i ~/.ssh/deploy_key
...
ssh_opts=(-p "$VPS_PORT" "${common_opts[@]}")
scp_opts=(-P "$VPS_PORT" "${common_opts[@]}")
...
scp "${scp_opts[@]}" "$SCRIPT_DIR/deploy/publish-native-release.sh" "$VPS_USER@$VPS_HOST:$REMOTE_DIR/publish-native-release.sh"
# Copy staged files file-by-file so spaces/newlines in names cannot break the upload.
while IFS= read -r -d '' file; do
  name="$(basename "$file")"
  scp "${scp_opts[@]}" "$file" "$VPS_USER@$VPS_HOST:$REMOTE_DIR/files/$name"
