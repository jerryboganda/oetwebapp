...
# RESEARCHER â€” Codebase exploration: patterns, dependencies, architecture discovery.
...
- Identify focus_area
- Research Pass â€” Pattern discovery:
...
- Early Exit:
  - If confidence â‰¥ 0.85 â†’ skip relationships + detailed â†’ Synthesize Phase.
  - If decision_blockers resolved AND confidence â‰¥ 0.8 â†’ early exit.
  - Else â†’ continue.
- Output:
...
```json
{
  ...
  "objective": "string",
  "focus_area": "string",
  "tldr": "string â€” dense bullet summary",
  "research_metadata": {
    "methodology": "string â€” e.g., semantic_search+grep_search, Context7",
    "scope": "string",
...
confidence = base(0.2) Ã— coverage_score(0.3) Ã— pattern_score(0.25) Ã— quality_score(0.25)
...
- quality_score: has_architecture(+0.2) + has_dependencies(+0.2) + has_open_questions(+0.1)
  Early exit: confidenceâ‰¥0.85 OR (confidenceâ‰¥0.8 AND decision_blockers resolved).
