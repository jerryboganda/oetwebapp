...
| # | Gap | Evidence | Fix sketch | Resolution (2026-05-12) |
|---|---|---|---|---|
| P0-1 | Writing AI grader is a hard-coded stub. Every learner gets `ScoreRange="330-360"`, `GradeRange="C+-B"`, identical strengths/issues/criterion scores, identical anchored feedback item. None of the rulebook is consulted. | `backend/src/OetWithDrHesham.Api/Services/BackgroundJobProcessor.cs#L408-L460` (`CompleteWritingEvaluationAsync`). Comment explicitly states "placeholder values for the mock evaluation pipeline". | Replace stub with a real call to `AiGatewayService` using feature `writing.grade`; parse JSON contract at `AiGatewayService.cs#L843-L856` into `Evaluation.CriterionScoresJson`/`FeedbackItemsJson`; gate via existing `AiQuotaService`. Run `WritingRuleEngine.cs` deterministic checks first and merge findings (see C-1 below). | ✅ **RESOLVED on `main` via commit `9d8003c` (`feat(launch): complete writing compliance and release gates`).** New `Services/Writing/WritingEvaluationPipeline.cs` (846 lines) replaces the stub. Calls `IAiGatewayService.BuildGroundedPrompt` → `CompleteAsync` with `FeatureCode = AiFeatureCodes.WritingGrade`, runs `WritingRuleEngine` pre-lint, merges findings (rule-engine wins), persists scores via `OetScoring.GradeWriting(scaled, candidateCountry)`. AI kill-switch via `IWritingOptionsProvider`. Validated by `WritingEvaluationPipelineTests` (2/2). Hardened in `3916e94` (prompt-compliant `passRequires` JSON tolerance). Test corpus 53/53 in `WritingRuleEngineTests`/`WritingEngineParityTests`. |

### P1 — High-impact gaps

| # | Gap | Evidence | Fix | Resolution (2026-05-12) |
|---|---|---|---|---|
| P1-1 | Deterministic `WritingRuleEngine` (40+ critical-rule detectors) is never invoked at submission time. The TS twin `lib/rulebook/writing-rules.ts` runs in the editor for live lint, but its findings are not persisted to the `Evaluation`. | `backend/src/OetWithDrHesham.Api/Services/Rulebook/WritingRuleEngine.cs` is unused by `LearnerService.SubmitWritingAsync` / `BackgroundJobProcessor.CompleteWritingEvaluationAsync`. | After P0-1: invoke `WritingRuleEngine.Evaluate(letter, letterType, profession)` synchronously in `CompleteWritingEvaluationAsync`, merge with AI findings, dedupe by `ruleId`, persist. | ✅ **RESOLVED on `main` via `9d8003c`.** `WritingEvaluationPipeline` invokes `WritingRuleEngine.Evaluate` synchronously before the AI call; deterministic findings merge with AI findings (rule-engine wins on `ruleId` collision); persisted to `Evaluation.FeedbackItemsJson`. 41/41 .NET detectors locked by `WritingEngineParityTests` against the TS engine. |
| P1-2 | AI prompt sends only the first 60 MAJOR rules verbatim and truncates the rest with "… and N more major rules." — even after wiring P0-1, the AI cannot assess rules 61+ in a single call. | `backend/src/OetWithDrHesham.Api/Services/Rulebook/AiGatewayService.cs#L798-L800` — `major.Take(60)`. | Either (a) raise to full corpus and accept higher token cost, or (b) split into two grader calls (CRITICAL+first-half MAJOR; CRITICAL+second-half MAJOR) and merge findings. Decision lives with cost tradeoff. | ✅ **RESOLVED on `main` via `824f638` (Momus review fixes B1, F1-F6).** Hybrid grader: `AiGatewayService` now sends the full critical+major corpus via the structured rulebook adjudication path, and the deterministic `WritingRuleEngine` runs first so AI tokens are spent only on semantic rules that need adjudication. Coverage matrix in `lib/rulebook/writing-coverage.ts` records adjudication path per rule. |
| P1-3 | Frontend `WRITING_LETTER_TYPES` (Referral / Discharge / Transfer / Advice / Update) does not match the canonical 6 backend types (`routine_referral` / `urgent_referral` / `non_medical_referral` / `update_discharge` / `update_referral_specialist_to_gp` / `transfer_letter`). The classifier in `lib/rulebook/context.ts` returns canonical codes; the UI workflow uses display-only labels. Risk: rule-applicability filter mis-matches. | `lib/writing/workflow.ts#L12-L20` vs `backend/src/OetWithDrHesham.Api/Services/Content/WritingContentStructure.cs#L17-L60`. | Single source of truth: import canonical 6 from a shared TS const; render display labels from a separate map. | ✅ **RESOLVED on `main` via `d144967` (`feat(writing,reading): hybrid grader + reading enforcement + letter-type SSO`).** Letter-type SSO from canonical TS const; display labels in a separate map. Profession-letter-type publish gate hardened in `64d7fc1`. |

### P2 — Medium-impact gaps

| # | Gap | Evidence | Fix | Resolution (2026-05-12) |
|---|---|---|---|---|
| P2-1 | `WritingCoachService` (live in-editor coach) is regex-stub only. Does not surface deterministic rule violations from `WritingRuleEngine`. | `backend/src/OetWithDrHesham.Api/Services/WritingCoachService.cs#L14-L100`. | Replace regex stubs by delegating to `WritingRuleEngine` for the live editor, returning a debounced subset (sentence-length, contractions, "the patient", Latin abbreviations). | ✅ **RESOLVED on `main` via `9d8003c`.** `WritingCoachService` rewritten (511-line change) to delegate to `WritingRuleEngine` for deterministic findings + AI suggestions through the gateway, deduped by `ruleId+anchor`, kill-switch honoured. Validated by `WritingCoachServiceTests` (16+ tests). |
| P2-2 | No first-class `rule_violations` table — violations buried in `Evaluation.FeedbackItemsJson`. Limits per-rule analytics ("which rule fails most often across cohort?"). | Schema in `backend/src/OetWithDrHesham.Api/Domain/Entities.cs#L349-L380`. | Add `WritingRuleViolation { Id, EvaluationId, RuleId, Severity, Quote, FixSuggestion, CreatedAt }`. Migration + index on `(RuleId, CreatedAt)`. | ✅ **RESOLVED 2026-05-13.** `WritingRuleViolation` persistence, analytics endpoint, migration, and focused tests are live. |
| P2-3 | No e2e test that submits a non-compliant letter and asserts rulebook-driven feedback is produced. Existing Playwright specs are smoke/nav only. | `tests/e2e/prod-exhaustive.spec.ts#L109-L118` and similar — no rule assertions. | Add `tests/e2e/writing/rulebook-grading.spec.ts` with fixtures violating each CRITICAL rule (contractions, "the patient", Latin abbrev, missing urgent closure phrase, "Date:" prefix, etc.) and assert the deterministic engine reports them. | ✅ **RESOLVED on `main` via `9d8003c`.** New `tests/e2e/learner/writing-rule-engine-violations.spec.ts` (38 tests across 6 projects) submits a violating letter and asserts rule-cited findings render in `/writing/feedback`. Smoke verified on production: `POST /v1/writing/lint` against a synthetic violating urgent-referral letter returned 26 rule findings (`R13.4`, `R13.3`, `R13.2`, `R09.2`, `R07.6`, `R07.3`, `R05.8`, `R12.1`, `R12.2`, `R08.14`, `R13.10`, etc.). |
| P2-4 | Reading rulebook B section 7 ("rules and regulations your academy must reflect") is partially implemented. "No use/disclosure of real test content" academic-integrity policy exists in terms but isn't surfaced inside the practice-test mode (no inline reminder). | `app/terms/page.tsx` covers it generically; no inline mock-mode policy banner. | Add a one-line policy line to the Part A intro screen and to mock-mode start screens. | ✅ **RESOLVED on `main` via 2026-05-12 closure.** Inline academic-integrity reminder added to the Reading attempt pre-start screen (`app/reading/paper/[paperId]/page.tsx`), the mock-mode banner section, and the mocks landing intro (`app/mocks/page.tsx`). Copy: "OET test content is confidential — do not redistribute or share questions outside this practice context." |
| P2-5 | Reading rulebook B section 6 ("Estimated OET band ... 'This is an estimate, not an official OET conversion.'") — present in code (`lib/scoring.ts#L780-L795`) but the literal disclaimer string was not verified rendered on `/reading/paper/.../results`. | `app/reading/paper/[paperId]/results/page.tsx#L99-L260`. | Verify and, if missing, append the disclaimer to the score card. | ✅ **RESOLVED on `main` via 2026-05-12 closure.** Literal disclaimer string `"This is an estimate, not an official OET conversion."` now renders verbatim on `/reading/paper/[paperId]/results` inside the score-card aside, gated to non-practice attempts (where a scaled OET score is reported). |

### P3 — Lower priority / informational

| # | Gap | Evidence | Resolution (2026-05-12) |
|---|---|---|---|
| P3-1 | Reading rulebook A R10.10 — "Zoom + and - buttons within the exam application; browser Ctrl+ blocked". Code provides app-level zoom controls (`ReadingZoomControls`, 80–125%) but does not block browser Ctrl+/Ctrl-. Acceptable for a web SPA; add a "use the in-app zoom" tooltip. | `app/reading/paper/[paperId]/page.tsx#L656-L673`. | ✅ **RESOLVED on `main` via 2026-05-12 closure.** `ReadingZoomControls` now exposes a hint label ("Use the in-app zoom; browser Ctrl+/- may misalign the timer.") via `aria-describedby`, plus a visually-hidden caption rendered for screen-reader users. The pre-existing `useReadingBrowserZoomGuard()` hook continues to swallow Ctrl-wheel zoom inside the player. |
| P3-2 | Reading rulebook B section 11 — content quantity targets (20–30 full mocks, 50+ Part A standalone, etc.) — content-quantity audit not in scope of code; track in admin analytics. | n/a. | ⚪ **OUT OF CODE SCOPE — content team work.** Tracked in `docs/STATUS/admin-route-inventory.md` and `docs/audits/WRITING-DRILL-PROFESSION-COVERAGE.md`. |
| P3-3 | Writing rulebook drills only populated for the medicine profession. The 11 non-medicine professions have empty drill banks. | `rulebooks/writing/drills/medicine/**` only. | ✅ **RESOLVED 2026-05-13.** `lib/writing-drills/loader.ts` now registers medicine seeds, all non-medicine abbreviation seeds, and generated profession-specific relevance/opening/ordering/expansion/tone drills. `lib/writing-drills/__tests__/loader.test.ts` locks all 12 professions x 6 drill types. |
...
| R01.2 | 42 Q total, A=20/B=6/C=16 | ✅ | `ReadingPart.MaxRawScore` enforced 20/6/16; counts gate in `ReadingStructureService` [tests #L110-L145](../../oetwebapp/backend/tests/OetWithDrHesham.Api.Tests/ReadingAuthoringTests.cs#L110-L145). |
| R01.3 | A standalone 15 min; B+C shared 45 min | ✅ | `PartABreakMaxSeconds=600` + deadlines [`ReadingAttemptService.cs#L221-L237`](../../oetwebapp/backend/src/OetWithDrHesham.Api/Services/Reading/ReadingAttemptService.cs#L221-L237). |
...
| R02.1 | 30/42 = B = 350 | ✅ | [`lib/scoring.ts#L286`](../../oetwebapp/lib/scoring.ts#L286) anchor; [`OetScoring.OetRawToScaled`](../../oetwebapp/backend/src/OetWithDrHesham.Api/Services/OetScoring.cs). |
...
| 3 | Part A simulator features (15-min lock, A–D layout, auto-submit, no return, exact-answer, accepted-variants, wrong-answer diagnosis) | ✅ | All 7 sub-features present. Wrong-answer diagnosis = distractor categorisation [`ReadingDistractorCategory`](../../oetwebapp/backend/src/OetWithDrHesham.Api/Domain/ReadingEntities.cs#L98-L105). |
