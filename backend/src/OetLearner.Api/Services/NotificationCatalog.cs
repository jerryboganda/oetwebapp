...
public static class NotificationCatalog
{
    ...
    public static string BuildTitle(NotificationEventKey key, IReadOnlyDictionary<string, string?> tokens)
        => key switch
        {
            ...
            NotificationEventKey.LearnerMockScheduled => "Your Full Mock Speaking session is confirmed",
            NotificationEventKey.ExpertSpeakingSessionAssigned => "A Full Mock Speaking session is assigned to you",
            ...
            NotificationEventKey.AdminAiBudgetAlert => "AI budget threshold reached",
    ...
    public static string BuildBody(NotificationEventKey key, IReadOnlyDictionary<string, string?> tokens)
        => key switch
        {
            ...
            NotificationEventKey.LearnerMockScheduled => $"Your session is scheduled for {ReadToken(tokens, "scheduledStartAt", "the confirmed time")}. Join from {ReadToken(tokens, "joinUrl", "your speaking room")}. Zoom: {ReadToken(tokens, "zoomJoinUrl", "the room link will appear when ready")}.",
            NotificationEventKey.ExpertSpeakingSessionAssigned => $"A Full Mock Speaking session is scheduled for {ReadToken(tokens, "scheduledStartAt", "the confirmed time")}. Open booking {ReadToken(tokens, "bookingId", string.Empty)} to start the tutor session.",
            ...
            NotificationEventKey.AdminAiBudgetAlert => ReadToken(tokens, "message", $"AI budget reached {ReadToken(tokens, "thresholdPct", "—")}% for {ReadToken(tokens, "scope", "scope")} ({ReadToken(tokens, "periodKey", "period")})."),
    ...
    public static string? BuildActionUrl(NotificationEventKey key, IReadOnlyDictionary<string, string?> tokens)
        => key switch
        {
            ...
            NotificationEventKey.AdminAiBudgetAlert => "/admin/ai-usage",
    ...
    private static IReadOnlyCollection<NotificationCatalogEntry> BuildEntries()
    {
        ...
        [
            ...
            new(NotificationEventKey.AdminAiBudgetAlert, ApplicationUserRoles.Admin, "operations", "AI Budget Alert", "Notify admins when AI provider budget thresholds are reached. Separate from billing/invoice failures.", NotificationSeverity.Warning, new(true, true, false, NotificationEmailMode.Immediate)),
            ...
            new(NotificationEventKey.LearnerLiveClassCancelled, ApplicationUserRoles.Learner, "live_classes", "Live Class Cancelled", "Notify learners when a reserved live class is cancelled.", NotificationSeverity.Warning, new(true, true, true, NotificationEmailMode.Immediate)),
            new(NotificationEventKey.LearnerLiveClassReminder, ApplicationUserRoles.Learner, "live_classes", "Live Class Reminder", "Remind learners before a reserved live class starts.", NotificationSeverity.Info, new(true, true, true, NotificationEmailMode.Immediate)),
            new(NotificationEventKey.LearnerLiveClassRecordingReady, ApplicationUserRoles.Learner, "live_classes", "Live Class Recording Ready", "Notify learners when a live-class replay and notes are available.", NotificationSeverity.Success, new(true, true, true, NotificationEmailMode.Immediate)),
            new(NotificationEventKey.ExpertLiveClassAssigned, ApplicationUserRoles.Expert, "live_classes", "Live Class Assigned", "Notify experts when they are assigned to a live class.", NotificationSeverity.Info, new(true, true, true, NotificationEmailMode.Immediate)),
            new(NotificationEventKey.ExpertLiveClassReminder, ApplicationUserRoles.Expert, "live_classes", "Live Class Reminder", "Remind experts before an assigned live class starts.", NotificationSeverity.Info, new(true, true, true, NotificationEmailMode.Immediate)),
            new(NotificationEventKey.AdminLiveClassSessionAlert, ApplicationUserRoles.Admin, "operations", "Live Class Session Alert", "Notify admins about live-class session failures, cancellations, and webhook issues.", NotificationSeverity.Warning, new(true, true, false, NotificationEmailMode.Immediate)),
            ...
            new(NotificationEventKey.LearnerMockReminder1h, ApplicationUserRoles.Learner, "mocks", "Mock Reminder 1h", "Remind learners approximately one hour before a mock exam.", NotificationSeverity.Info, new(true, true, true, NotificationEmailMode.Immediate)),
            new(NotificationEventKey.ExpertMockReminder1h, ApplicationUserRoles.Expert, "mocks", "Mock Reminder 1h", "Remind assigned experts approximately one hour before a mock exam.", NotificationSeverity.Info, new(true, true, true, NotificationEmailMode.Immediate)),
            new(NotificationEventKey.LearnerMockReminder15m, ApplicationUserRoles.Learner, "mocks", "Mock Reminder 15m", "Remind learners fifteen minutes before a mock exam.", NotificationSeverity.Warning, new(true, true, true, NotificationEmailMode.Immediate)),
            new(NotificationEventKey.ExpertMockReminder15m, ApplicationUserRoles.Expert, "mocks", "Mock Reminder 15m", "Remind assigned experts fifteen minutes before a mock exam.", NotificationSeverity.Warning, new(true, true, true, NotificationEmailMode.Immediate)),
