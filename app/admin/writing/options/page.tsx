    ...
    <div className="flex items-start justify-between gap-4 py-3">
      <div className="min-w-0">
        <div className="flex items-center gap-2">
          <span className="text-sm font-medium text-admin-fg-strong">{label}</span>
          {value ? (
...
export default function WritingOptionsPage() {
  ...
  return (
    <AdminSettingsLayout
      title="Writing AI Options"
      description="Control the Writing AI kill-switch (grading + coach), the free-tier entitlement that gates non-paying learners, and global learner access. Changes apply immediately."
      breadcrumbs={BREADCRUMBS}
      eyebrow="Writing"
      icon={<PenSquare className="h-5 w-5" />}
      actions={(
    ...
    >
      ...
      <SettingsSection
        ...
        description="Disabling either toggle immediately stops the corresponding Writing AI feature for every learner. Record a brief reason so future admins understand why it was turned off."
      >
        ...
        {killSwitchActive && (
          ...
          >
            <label className="block">
              <span className="text-xs font-medium text-admin-fg-strong">
                Kill-switch reason
              </span>
              <textarea
      ...
      <SettingsSection
        title="Entitlement (Free Tier)"
        description="By default Writing AI is premium-only. Turn the free tier on to grant non-paying learners a quota per rolling window. A limit of 0 keeps access premium-only even when the tier is enabled."
      >
        <div className="divide-y divide-admin-border">
          <ToggleRow
